...
// - Storage: SharedPreferences StringSet \"user_categories\" (matches
existing code)

// - Room category column on RecentFileEntity holds full path e.g.
\"Work/Invoices\"
...
// All category paths, sorted. e.g. [\"General\", \"Work\",
\"Work/Invoices\"]
...
// e.g. getChildren(\"Work\") -> [\"Invoices\", \"Reports\"] (not
grandchildren)
...
val fullPath = if (parentPath.isBlank()) cleanName else
\"\$parentPath/\$cleanName\"

val prefs = prefs(context)

val current = (prefs.getStringSet(KEY_USER_CATS, emptySet()) ?:
...
// Note: files in the deleted category are moved to \"General\" in the
Room DB --
...
// Remove exact path AND all descendants (paths starting with
\"fullPath/\")
...
fun renameCategory(context: Context, oldPath: String, newName: String):
String {
...
val newPath = if (parentPath.isBlank()) cleanName else
\"\$parentPath/\$cleanName\"
...
fun moveCategory(context: Context, fullPath: String, newParentPath:
String): String {

val leafName = leafOf(fullPath)

val newPath = if (newParentPath.isBlank()) leafName else
\"\$newParentPath/\$leafName\"
...
// Ensure all ancestor paths are present (so \"Work/Invoices\" implies
\"Work\" exists)

private fun ensureAncestors(context: Context, path: String, set:
MutableSet<String>) {
...
**AppShortcutsHelper.kt**

Long-press app icon shortcuts via ShortcutManagerCompat. Action
constants defined internally (FIX: no dependency on undefined
MainActivity constants).

**Deployed to:** app/src/main/java/com/propdf/editor/utils/
