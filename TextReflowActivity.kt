// FILE: TextReflowActivity.kt (v4.1 -- build-error-fixed)
// FLAT REPO ROOT -- codemagic.yaml copies to:
// app/src/main/java/com/propdf/editor/ui/viewer/TextReflowActivity.kt
//
// FIX: Removed @Inject lateinit var fileHelper: FileHelper
//      FileHelper is an 'object' singleton -- Hilt cannot inject object singletons.
//      Use FileHelper.uriToFile(...) directly (static-style call).
//
// RULES OBEYED:
//   Rule #9:  setTextIsSelectable(true) method call -- NOT val assignment
//   Rule #14: isVerticalScrollBarEnabled = false
//   Rule #10: FrameLayout.LayoutParams(w, h) -- no weight float
//   Rule #32: Pure ASCII
...
class TextReflowActivity : AppCompatActivity() {
    ...
    // FIX: FileHelper is an 'object' -- no @Inject. Use FileHelper.method() directly.
    ...
    private fun renderPages() {
        ...
        for (page in pages) {
            ...
            // Text view -- rule #9: setTextIsSelectable(true) method, NOT = assignment
    ...
    private fun doSearch(query: String) {
        ...
        tvSearchResult.text = if (totalMatches == 0) "Not found" else "$totalMatches result(s)"
    ...
    private fun clearSearch() {
        ...
        etSearch.text?.clear()
        tvSearchResult.text = ""
        textViews.forEachIndexed { idx, tv -> tv.text = pages.getOrNull(idx)?.text ?: "" }
    }
