...
# Check 3: Verify the broken KAPT "workarounds" are NOT in gradle.properties.
# kapt.include.compile.classpath=false stops :app's kapt from loading its
# dependency modules' compiled classes, and kotlin.stdlib.default.dependency=false
# stops Gradle from auto-supplying a matching kotlin-stdlib - together these are
# what actually produces "Could not load module <Error module>", not a fix for it.
echo "[3/8] Checking harmful KAPT/stdlib properties are absent from gradle.properties..."
for prop in "kapt.include.compile.classpath=false" "kotlin.stdlib.default.dependency=false"; do
    if rg -q --fixed-strings "$prop" gradle.properties 2>/dev/null; then
        fail "Found harmful property (should be removed): $prop"
    else
        pass "Confirmed absent: $prop"
    fi
done
