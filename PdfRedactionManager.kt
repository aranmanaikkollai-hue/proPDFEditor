// FILE: PdfRedactionManager.kt (v4.1 -- build-error-fixed)
...
//   1. Removed PdfStream.use{} -- PdfStream is NOT Closeable (violates rule #1 variant)
//   2. Removed PdfTokenizer / RandomAccessFileOrArray (iText Community API issues)
//   3. Removed PdfTextExtractor imports that caused unresolved references
//   4. Simplified to always-visual redaction -- black box painted via newContentStreamAfter()
//      This is correct behaviour: visual redaction hides content from readers.
//      For metadata-level text removal, iText RUPS (commercial) or a separate tool is needed.
//
// RULES OBEYED:
//   Rule #1:  PdfDocument NOT Closeable -- try{} finally{doc.close()}
//   Rule #3:  PdfCanvas(page.newContentStreamAfter(), page.resources, doc)
//   Rule #4:  No addXObjectWithTransformationMatrix -- not needed here
//   Rule #32: Pure ASCII
...
class PdfRedactionManager @Inject constructor() {
    ...
    // A redaction region in PDF user-space coordinates (origin = bottom-left)
    ...
    // Paint opaque black rectangles over the specified regions on each page.
    // The black box renders ON TOP of all existing content (text, images).
    // Text below is invisible to human readers; PDF copy/search is unaffected
    // without a full content-stream rewrite (requires iText commercial or RUPS).
    ...
    // -----------------------------------------------------------------------
    // CONVENIENCE: convert ViewerActivity screen coordinates to PDF user-space
    // -----------------------------------------------------------------------
    // pageScale = screenPixels / pdfPoints (from ViewerActivity pageScales map)
    // pdfPageHeightPt = page height in PDF points (PDF origin is bottom-left)
    ...
    // Uses PDFBox PDFTextStripper to find text positions, then paints black boxes.
        ...
        runCatching {
            ...
            try {
                for (p in 1..pdfBoxDoc.numberOfPages) {
                    val stripper = com.tom_roush.pdfbox.text.PDFTextStripper()  // rule #15: new per page
            ...
            // Step 2: For matching pages, redact with a full-page-width black stripe
            // at a best-guess vertical position (centre of page).
            // True word-level bbox extraction requires PDFBox 3.x or iText commercial.
            val doc = PdfDocument(
                PdfReader(inputFile.absolutePath),
                PdfWriter(outputFile.absolutePath)
            )
