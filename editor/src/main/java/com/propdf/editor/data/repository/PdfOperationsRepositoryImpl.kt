...
class PdfOperationsRepositoryImpl @Inject constructor(
    ...
    override suspend fun deletePages(sourceUri: Uri, pages: List<Int>): AppResult<File> {
        val input = runCatching { resolveUriToFile(sourceUri) }
            .getOrElse { return AppResult.Error(it.toAppException()) }
        return deletePages(input, createOutputFile("deleted"), pages)
    }

    override suspend fun duplicatePages(sourceUri: Uri, pages: List<Int>): AppResult<File> =
        withContext(dispatchers.io) {
            runCatching {
    ...
    override suspend fun movePages(sourceUri: Uri, pages: List<Int>, target: Int): AppResult<File> =
        withContext(dispatchers.io) {
            runCatching {
                val input = resolveUriToFile(sourceUri)
    ...
    override suspend fun extractPages(sourceUri: Uri, pages: List<Int>, outputName: String): AppResult<File> =
        withContext(dispatchers.io) {
            runCatching {
                val input = resolveUriToFile(sourceUri)
    ...
    override suspend fun rotatePages(sourceUri: Uri, pages: List<Int>, degrees: Int): AppResult<File> {
        val input = runCatching { resolveUriToFile(sourceUri) }
            .getOrElse { return AppResult.Error(it.toAppException()) }
        val rotations = pages.associateWith { degrees.toFloat() }
        return rotatePages(input, createOutputFile("rotated"), rotations)
    }

    override suspend fun cropPages(sourceUri: Uri, pages: List<Int>, config: CropConfig): AppResult<File> =
        withContext(dispatchers.io) {
            runCatching {
                val input = resolveUriToFile(sourceUri)
    ...
    override suspend fun resizePages(sourceUri: Uri, pages: List<Int>, config: ResizeConfig): AppResult<File> =
        withContext(dispatchers.io) {
            runCatching {
                val input = resolveUriToFile(sourceUri)
    ...
    override suspend fun mirrorPages(sourceUri: Uri, pages: List<Int>, horizontal: Boolean): AppResult<File> =
        withContext(dispatchers.io) {
            runCatching {
                val input = resolveUriToFile(sourceUri)
    ...
    override suspend fun insertBlankPage(sourceUri: Uri, position: Int, width: Float, height: Float): AppResult<File> =
        withContext(dispatchers.io) {
            runCatching {
                val input = resolveUriToFile(sourceUri)
    ...
    override suspend fun splitBySize(sourceUri: Uri, maxSizeMb: Int, outputPrefix: String): AppResult<List<File>> =
        withContext(dispatchers.io) {
            runCatching {
                val input = resolveUriToFile(sourceUri)
    ...
    override suspend fun splitByBookmark(sourceUri: Uri, outputPrefix: String): AppResult<List<File>> =
        withContext(dispatchers.io) {
            runCatching {
                val input = resolveUriToFile(sourceUri)
                val src = com.itextpdf.kernel.pdf.PdfDocument(PdfReader(input.absolutePath))
    ...
    override suspend fun splitEveryNPages(sourceUri: Uri, n: Int, outputPrefix: String): AppResult<List<File>> =
        withContext(dispatchers.io) {
            runCatching {
                val input = resolveUriToFile(sourceUri)
                val src = com.itextpdf.kernel.pdf.PdfDocument(PdfReader(input.absolutePath))
    ...
    override suspend fun addPageNumbers(sourceUri: Uri, config: PageNumberConfig): AppResult<File> {
        val input = runCatching { resolveUriToFile(sourceUri) }
            .getOrElse { return AppResult.Error(it.toAppException()) }
        return addPageNumbers(input, createOutputFile("numbered"), config)
    }

    override suspend fun addHeaderFooter(sourceUri: Uri, config: HeaderFooterConfig): AppResult<File> {
        val input = runCatching { resolveUriToFile(sourceUri) }
            .getOrElse { return AppResult.Error(it.toAppException()) }
        return addHeaderFooter(input, createOutputFile("headerfooter"), config)
    }

    override suspend fun addWatermark(sourceUri: Uri, config: WatermarkConfig): AppResult<File> {
        val input = runCatching { resolveUriToFile(sourceUri) }
            .getOrElse { return AppResult.Error(it.toAppException()) }
        return addWatermark(input, createOutputFile("watermarked"), config)
    }

    override suspend fun addBackground(sourceUri: Uri, config: BackgroundConfig): AppResult<File> =
        withContext(dispatchers.io) {
            runCatching {
                val input = resolveUriToFile(sourceUri)
    ...
    override suspend fun compressPdf(sourceUri: Uri, config: CompressConfig): AppResult<File> {
        val input = runCatching { resolveUriToFile(sourceUri) }
            .getOrElse { return AppResult.Error(it.toAppException()) }
        return compress(input, createOutputFile("compressed"), config)
    }

    override suspend fun optimizePdf(sourceUri: Uri, aggressive: Boolean): AppResult<File> {
        val input = runCatching { resolveUriToFile(sourceUri) }
            .getOrElse { return AppResult.Error(it.toAppException()) }
        ...
        return compress(input, createOutputFile("optimized"), config)
    }

    // ===================== URI-BASED TOOL-SCREEN OPERATIONS =====================

    override suspend fun insertImagePage(sourceUri: Uri, position: Int, config: ImageInsertionConfig): AppResult<File> {
        val input = runCatching { resolveUriToFile(sourceUri) }
            .getOrElse { return AppResult.Error(it.toAppException()) }
    ...
    override suspend fun combineImagesToPdf(images: List<Uri>, outputName: String, config: ImageInsertionConfig): AppResult<File> =
        withContext(dispatchers.io) {
            runCatching {
