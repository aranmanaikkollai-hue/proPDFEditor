...
class MainActivity : AppCompatActivity() {
    ...
    // FIX: separate field for tracking which category/subcategory is drilled into
    private var catDetailName  = ""
    private var catDetailDepth = 0   // 0=top, 1=sub
    ...
    private fun buildSortViewRow(): LinearLayout {
        return LinearLayout(this).apply {
            ...
            sortBtn = TextView(this@MainActivity).apply { layoutParams = LinearLayout.LayoutParams(0, -2, 1f); text = sortLabel(); textSize = 11f; typeface = Typeface.DEFAULT_BOLD; setTextColor(Color.parseColor(txt2())); setOnClickListener { showSortMenu() } }
            addView(sortBtn)
            viewToggleBtn = ImageButton(this@MainActivity).apply {
                layoutParams = LinearLayout.LayoutParams(dp(34), dp(34)); setImageResource(viewToggleIcon()); colorFilter = PorterDuffColorFilter(c_pri, PorterDuff.Mode.SRC_IN); setBackgroundColor(Color.TRANSPARENT)
    ...
    private fun buildSectionRow(): LinearLayout {
        return LinearLayout(this).apply {
            orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER_VERTICAL; setPadding(dp(20), dp(14), dp(20), dp(6)); setBackgroundColor(bg())
            tvSection = TextView(this@MainActivity).apply {
                ...
                setTextColor(Color.parseColor(txt1())); letterSpacing = if (isDark) 0f else 0.12f; layoutParams = LinearLayout.LayoutParams(0, -2, 1f)
            }
            addView(tvSection)
        }
    }
    ...
    private fun buildBottomNav(): FrameLayout {
        ...
        listOf(NI("home","HOME",android.R.drawable.ic_menu_view), NI("tools","TOOLS",android.R.drawable.ic_menu_preferences), NI("__fab__","",0), NI("scan","SCAN",android.R.drawable.ic_menu_camera), NI("ocr","OCR",android.R.drawable.ic_menu_edit)).forEach { item ->
        ...
        val fab = FrameLayout(this).apply {
            ...
            setBackgroundColor(c_pri); outlineProvider = object : ViewOutlineProvider() { override fun getOutline(v: View, o: android.graphics.Outline) { o.setOval(0, 0, v.width, v.height) } }
    ...
    private fun switchTab(tab: String) {
        currentTab = tab
        if (tab != "cat_detail") { catDetailName = ""; catDetailDepth = 0 }
        updateTabHighlight(tab); updateNavHighlight(tab); rebuildFileList()
    }

    private fun updateTabHighlight(tab: String) {
        val highlightTab = if (tab == "cat_detail") "categories" else tab
        listOf("recent","starred","categories","bookmarks").forEach { t ->
            (tabRow.findViewWithTag("${t}_tv") as? TextView)?.apply {
                setTextColor(if (t == highlightTab) tabActTxt() else tabInaTxt())
                typeface = if (t == highlightTab) Typeface.DEFAULT_BOLD else Typeface.DEFAULT
                setBackgroundColor(if (isDark && t == highlightTab) tabPill() else Color.TRANSPARENT)
            }
        }
    }
    ...
    private fun updateNavHighlight(tab: String) {
        ...
        listOf("home","tools","scan","ocr").forEach { id ->
            ...
            (rootFrame.findViewWithTag("${id}_icon") as? ImageView)?.setColorFilter(if (active) c_pri else Color.parseColor(txt2()))
            (rootFrame.findViewWithTag("${id}_label") as? TextView)?.setTextColor(if (active) c_pri else Color.parseColor(txt2()))
        }
    }
    ...
    private fun observeFiles() {
        lifecycleScope.launch {
            db.recentFilesDao().getAll().collect { files ->
                allFileEntities = files; rebuildFileList()
            }
        }
    }
    ...
    private fun sortLabel(): String { val dir = if (sortAsc) "\u25b2" else "\u25bc"; return when (sortMode) { "name" -> "Name $dir"; "size" -> "Size $dir"; else -> "Date $dir" } }
    ...
    private fun showSortMenu() {
        AlertDialog.Builder(this).setTitle("Sort by").setItems(arrayOf("Date (newest)","Date (oldest)","Name A-Z","Name Z-A","Size (large)","Size (small)")) { _, which ->
            when (which) { 0->{sortMode="date";sortAsc=false}; 1->{sortMode="date";sortAsc=true}; 2->{sortMode="name";sortAsc=false}; 3->{sortMode="name";sortAsc=true}; 4->{sortMode="size";sortAsc=false}; 5->{sortMode="size";sortAsc=true} }
            if (::sortBtn.isInitialized) sortBtn.text = sortLabel(); rebuildFileList()
        }.show()
    }

    private fun sortedFiles(): List<RecentFileEntity> {
        // FIX: cat_detail shows ONLY direct files of that exact category path
        ...
        return when (sortMode) {
            "name" -> if (sortAsc) base.sortedByDescending { it.displayName.lowercase(Locale.getDefault()) } else base.sortedBy { it.displayName.lowercase(Locale.getDefault()) }
    ...
    private fun sortedFilesForCat(files: List<RecentFileEntity>): List<RecentFileEntity> {
        return when (sortMode) {
            "name" -> if (sortAsc) files.sortedByDescending { it.displayName.lowercase(Locale.getDefault()) } else files.sortedBy { it.displayName.lowercase(Locale.getDefault()) }
            "size" -> if (sortAsc) files.sortedBy { it.fileSizeBytes } else files.sortedByDescending { it.fileSizeBytes }
            else   -> if (sortAsc) files.sortedBy { it.lastOpenedAt } else files.sortedByDescending { it.lastOpenedAt }
        }
    }
    ...
    private fun rebuildFileList() {
        ...
        if (::tvSection.isInitialized) {
            tvSection.text = when (currentTab) {
                "recent"     -> if (isDark) "Recent Files" else "LAST ACCESSED"
                "starred"    -> "Starred"
                "categories" -> "My Vault"
                "cat_detail" -> catDetailName.substringAfterLast("/")
                "bookmarks"  -> "Bookmarks"
                else         -> tvSection.text.toString()
            }
            tvSection.textSize = if (isDark) 20f else 9f
        }
        // FIX: cat_detail -- show ONLY files in that exact folder with back button breadcrumb
        if (currentTab == "cat_detail") {
            renderCatDetail(); return
        }
        ...
        tvEmpty.text = when (currentTab) { "starred" -> "No starred files"; "bookmarks" -> "No bookmarks yet"; else -> "No files yet\nTap + to open a PDF" }
    ...
    private fun renderCatDetail() {
        // Breadcrumb back button
        fileListContainer.addView(TextView(this).apply {
            val parent = if (catDetailName.contains("/")) catDetailName.substringBeforeLast("/") else ""
            text = "< ${if (parent.isEmpty()) "Vault" else parent.substringAfterLast("/")}"; textSize = 12f
            setTextColor(c_pri); typeface = Typeface.DEFAULT_BOLD; setPadding(0, 0, 0, dp(8))
            setOnClickListener {
                if (parent.isEmpty()) { currentTab = "categories"; catDetailName = ""; catDetailDepth = 0 }
                else { catDetailName = parent; catDetailDepth -= 1 }
                updateTabHighlight(currentTab); rebuildFileList()
            }
        })
        val files = sortedFiles()
        // FIX: show sub-folders of this category too
        val saved = prefs.getStringSet("user_categories", emptySet()) ?: emptySet()
        val subFolders = (allFileEntities.map { it.category } + saved)
            .filter { it.startsWith("$catDetailName/") && it.removePrefix("$catDetailName/").isNotEmpty() && !it.removePrefix("$catDetailName/").contains("/") }
            .distinct().sorted()
        subFolders.forEach { sub ->
            val subFiles = allFileEntities.filter { it.category == sub }
            fileListContainer.addView(buildVaultCard(sub, subFiles.size, 1))
        }
        if (files.isEmpty() && subFolders.isEmpty()) {
            tvEmpty.text = "No files in this folder"; tvEmpty.visibility = View.VISIBLE
        } else {
    ...
    private fun buildFileCard(f: RecentFileEntity): View {
        ...
        val thumb = FrameLayout(this).apply {
            ...
            outlineProvider = object : ViewOutlineProvider() { override fun getOutline(v: View, o: android.graphics.Outline) { o.setRoundRect(0, 0, v.width, v.height, dp(8).toFloat()) } }; clipToOutline = true
        }
    ...
    private fun buildFileCardCompact(entity: RecentFileEntity): LinearLayout {
        val card = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(dp(8), dp(8), dp(8), dp(8)); background = GradientDrawable().apply { setColor(Color.parseColor(card())); cornerRadius = dp(12).toFloat() }; elevation = dp(2).toFloat() }
        val tf = FrameLayout(this).apply { layoutParams = LinearLayout.LayoutParams(-1, dp(110)); setBackgroundColor(Color.parseColor("#F5F5F7")); outlineProvider = object : ViewOutlineProvider() { override fun getOutline(v: View, o: android.graphics.Outline) { o.setRoundRect(0, 0, v.width, v.height, dp(8).toFloat()) } }; clipToOutline = true }
    ...
    private fun buildFileTileRow(entity: RecentFileEntity): LinearLayout {
        return LinearLayout(this).apply {
            ...
            layoutParams = LinearLayout.LayoutParams(-1, -2).apply { bottomMargin = dp(4) }; background = GradientDrawable().apply { setColor(Color.parseColor(card())); cornerRadius = dp(8).toFloat() }
    ...
    private fun renderFirstPage(uriStr: String): Bitmap? {
        return try {
            val uri = Uri.parse(uriStr); val dest = File(cacheDir, "thumb_${uriStr.hashCode()}.pdf")
            ...
            Canvas(bmp).drawColor(Color.WHITE); page.render(bmp, null, null, PdfRenderer.Page.RENDER_MODE_FOR_DISPLAY)
    ...
    private fun renderVault() {
        ...
        // FIX: only top-level categories (no "/" in name)
        val topLevel = (fileCats.keys + saved).filter { it.isNotEmpty() && !it.contains("/") }.distinct().sorted()
        fileListContainer.addView(TextView(this).apply { text = "+ New Folder"; textSize = 12f; typeface = Typeface.DEFAULT_BOLD; setTextColor(c_pri); gravity = Gravity.END; setPadding(0, 0, dp(16), dp(8)); setOnClickListener { showCreateFolderDialog() } })
        ...
        topLevel.forEach { cat ->
            ...
            fileListContainer.addView(buildVaultCard(cat, directFiles.size, 0))
            // FIX: expanded top-level shows sub-folders + direct files
            if (expandedCategories.contains(cat)) {
                ...
                subFolders.forEach { sub ->
                    ...
                    fileListContainer.addView(buildVaultCard(sub, subFiles.size, 1))
                    // FIX: tapping sub-folder header drills into it (shows ONLY its files)
                    // This is handled by setOnClickListener in buildVaultCard
                }
                sortedFilesForCat(directFiles).forEach { f ->
                    fileListContainer.addView(buildFileTileRow(f).also { row ->
                        (row.layoutParams as? LinearLayout.LayoutParams)?.marginStart = dp(20)
                    })
                }
            }
        }
    ...
    private fun buildVaultCard(category: String, count: Int, depth: Int): LinearLayout {
        ...
        return LinearLayout(this).apply {
            ...
            addView(TextView(this@MainActivity).apply {
                layoutParams = LinearLayout.LayoutParams(dp(36), dp(36)); text = folderLetter; gravity = Gravity.CENTER; textSize = 14f; typeface = Typeface.DEFAULT_BOLD; setTextColor(c_acc)
            ...
            addView(tc)
            addView(TextView(this@MainActivity).apply { text = if (isExpanded && depth == 0) "\u25bc" else "\u25b6"; textSize = 13f; setTextColor(Color.parseColor(txt2())) })
            setOnClickListener {
                if (depth == 0) {
                    // Top-level: toggle expand/collapse
                    if (isExpanded) expandedCategories.remove(category) else expandedCategories.add(category)
                    rebuildFileList()
                } else {
                    // FIX: Sub-folder tap drills in -- shows ONLY that subfolder's files
                    catDetailName  = category
                    catDetailDepth = depth
                    currentTab     = "cat_detail"
                    updateTabHighlight("categories")
                    rebuildFileList()
                }
            }
    ...
    private fun showAddSubCat(parent: String) {
        ...
        AlertDialog.Builder(this).setTitle("New sub-folder under \"${parent.substringAfterLast("/")}\"").setView(et)
            .setPositiveButton("Add") { _, _ ->
                ...
                if (n.isNotEmpty()) { val s = prefs.getStringSet("user_categories", mutableSetOf())!!.toMutableSet(); s.add("$parent/$n"); prefs.edit().putStringSet("user_categories", s).apply(); expandedCategories.add(parent); rebuildFileList() }
    ...
    private fun confirmDeleteCat(cat: String) {
        AlertDialog.Builder(this).setTitle("Delete \"${cat.substringAfterLast("/")}\"?").setMessage("Files will move to General.")
    ...
    private fun showMoveToCategoryDialog(f: RecentFileEntity) {
        ...
        AlertDialog.Builder(this).setTitle("Move to Folder").setItems(saved.map { it.replace("/", " / ") }.toTypedArray()) { _, i ->
    ...
    private fun showFileOptions(f: RecentFileEntity) {
        ...
        AlertDialog.Builder(this).setTitle(f.displayName).setItems(ops) { _, w ->
            when (w) {
                ...
                4 -> AlertDialog.Builder(this).setTitle("Delete?").setMessage("Remove \"${f.displayName}\" from list?")
    ...
    private fun showRenameDialog(f: RecentFileEntity) {
        ...
        AlertDialog.Builder(this).setTitle("Rename").setView(et)
            .setPositiveButton("Save") { _, _ ->
                val n = et.text.toString().trim()
                if (n.isNotEmpty()) { val newName = if (n.endsWith(".pdf")) n else "$n.pdf"; lifecycleScope.launch(Dispatchers.IO) { db.recentFilesDao().insert(f.copy(displayName = newName)); withContext(Dispatchers.Main) { toast("Renamed to $newName") } } }
            }.setNegativeButton("Cancel", null).show()
    }
    ...
    private fun showSearchDialog() {
        ...
        et.addTextChangedListener(object : android.text.TextWatcher {
            override fun afterTextChanged(s: android.text.Editable?) {
                val q = s.toString().lowercase(Locale.getDefault()); fileListContainer.removeAllViews()
        ...
        AlertDialog.Builder(this).setTitle("Search Files").setView(et).setNegativeButton("Close") { _, _ -> rebuildFileList() }.show()
    }

    private fun showSettings() {
        AlertDialog.Builder(this).setTitle("Settings")
            .setItems(arrayOf("Clear Recent Files (keep starred/bookmarked/categorized)", "Delete ALL Files", "About")) { _, i ->
                when (i) {
                    0 -> AlertDialog.Builder(this).setTitle("Clear Recent?")
                        .setMessage("Removes ONLY non-starred, non-bookmarked, non-categorized files.")
                        .setPositiveButton("Clear") { _, _ -> lifecycleScope.launch(Dispatchers.IO) { clearRecentPreservingPinned() } }
                        .setNegativeButton("Cancel", null).show()
                    1 -> AlertDialog.Builder(this).setTitle("Delete ALL?").setMessage("Removes every file from the list.")
                        .setPositiveButton("Delete All") { _, _ -> lifecycleScope.launch(Dispatchers.IO) { db.recentFilesDao().clearAll() } }
                        .setNegativeButton("Cancel", null).show()
                    2 -> AlertDialog.Builder(this).setTitle("ProPDF Editor v4.2").setMessage("Production-grade PDF editor.\nAnnotation engine: data-driven.\nStorage: Room + MediaStore.").setPositiveButton("OK", null).show()
                }
            }.show()
    }
    ...
    private suspend fun clearRecentPreservingPinned() {
        ...
        allFileEntities.forEach { entity ->
            ...
            if (!pinned) db.recentFilesDao().updateLastOpened(entity.uri, 0L)
        }
    }
