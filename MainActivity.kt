...
class MainActivity : AppCompatActivity() {
    ...
    private val ocrPicker = registerForActivityResult(
    ...
    // ─────────────────────────────────────────────────────────
    // ROOT UI  (rebuilt fully on theme toggle)
    // ─────────────────────────────────────────────────────────
    private fun buildUI() {
    ...
    private fun buildHeader(): LinearLayout {
        return LinearLayout(this).apply {
            ...
            addView(FrameLayout(this@MainActivity).apply {
                val s = dp(46)
                layoutParams =
                    LinearLayout.LayoutParams(s, s).apply { setMargins(0, 0, dp(10), 0) }
            ...
            // Theme toggle button — TRUE toggle: persists, rebuilds all UI
    ...
    private fun buildTabBar(): LinearLayout {
        ...
        listOf(
            ...
            tabRow.addView(View(this).apply {
                layoutParams = LinearLayout.LayoutParams(dp(2), dp(1))
            })
        }
    ...
    private fun buildSectionRow(): LinearLayout {
        return LinearLayout(this).apply {
            ...
            tvSection = TextView(this@MainActivity).apply {
                ...
                textSize = if (isDark) 20f else 9f
                typeface = Typeface.DEFAULT_BOLD
    ...
    private fun buildBottomNav(): FrameLayout {
        ...
        val navBar = LinearLayout(this).apply {
            ...
            gravity = Gravity.BOTTOM
            layoutParams =
                FrameLayout.LayoutParams(-1, dp(68)).apply { gravity = Gravity.BOTTOM }
        ...
        listOf(
            ...
            val active = item.id == "home" &&
                ...
                "recent", "starred", "categories", "cat_detail", "bookmarks"
            )
            val col = LinearLayout(this).apply {
                ...
                setOnClickListener {
                    when (item.id) {
                        ...
                        "tools" -> startActivity(
                            Intent(this@MainActivity, ToolsActivity::class.java)
                        )
                        "scan" -> startActivity(
                            Intent(
                                this@MainActivity,
        ...
        fab.addView(ImageView(this).apply {
            ...
            setColorFilter(Color.WHITE)
            layoutParams =
                FrameLayout.LayoutParams(dp(26), dp(26)).apply { gravity = Gravity.CENTER }
        })
    ...
    private fun switchTab(tab: String) {
        ...
        listOf("recent", "starred", "categories", "bookmarks").forEach { t ->
            (tabRow.findViewWithTag<View>("${t}_tv") as? TextView)?.apply {
                ...
                setBackgroundColor(
                    if (isDark && t == tab) tabPill() else Color.TRANSPARENT
                )
            }
        }
    ...
    private fun updateNavHighlight(tab: String) {
        val atHome =
            tab in listOf("recent", "starred", "categories", "cat_detail", "bookmarks")
    ...
    // ─────────────────────────────────────────────────────────
    // THEME TOGGLE — true toggle persisted across activities
    // ─────────────────────────────────────────────────────────
    private fun toggleTheme() {
        isDark = !isDark
        prefs.edit().putBoolean("dark_mode", isDark).apply()
        thumbnailCache.clear()
        // Apply to ViewerActivity / ToolsActivity via AppCompatDelegate in ProPDFApp
        buildUI()
        observeFiles()
    }
    ...
    private fun sortLabel(): String {
        ...
        return when (sortMode) {
            "name" -> "Name $dir"; "size" -> "Size $dir"; else -> "Date $dir"
        }
    }
    ...
    // ── Bookmarks: stored as "filename|pageNumber" ────────────
    ...
    private fun buildFileCard(f: RecentFileEntity): View {
        ...
        // FIX: display actual filename, not numerical ID
    ...
    private fun buildFileCardCompact(entity: RecentFileEntity): LinearLayout {
        ...
        card.addView(TextView(this).apply {
            text = formatSize(entity.fileSizeBytes)
            textSize = 9f
            setTextColor(Color.parseColor(txt2()))
        })
        ...
        if (cached != null) {
            iv.setImageBitmap(cached); lv.visibility = View.GONE
        } else {
            lifecycleScope.launch(Dispatchers.IO) {
    ...
    private fun buildFileTileRow(entity: RecentFileEntity): LinearLayout {
        return LinearLayout(this).apply {
            ...
            addView(TextView(this@MainActivity).apply {
                ...
                setTextColor(Color.WHITE)
                background = GradientDrawable().apply {
                    setColor(Color.parseColor(DANGER))
                    cornerRadius = dp(4).toFloat()
                }
            })
            // FIX: use displayName, not URI numeric ID
            addView(TextView(this@MainActivity).apply {
                layoutParams =
            ...
            addView(TextView(this@MainActivity).apply {
                text =
                    "${formatSize(entity.fileSizeBytes)} \u2022 ${relativeTime(entity.lastOpenedAt)}"
                textSize = 10f
                setTextColor(Color.parseColor(txt2()))
                layoutParams =
    ...
    private fun renderFirstPage(uriStr: String): Bitmap? {
        return try {
            ...
            val page = rnd.openPage(0)
            val bmpW = 320
            val bmpH =
                (page.height.toFloat() / page.width.toFloat() * bmpW).toInt().coerceAtLeast(1)
            val bmp = Bitmap.createBitmap(bmpW, bmpH, Bitmap.Config.ARGB_8888)
            Canvas(bmp).drawColor(Color.WHITE)
            page.render(bmp, null, null, PdfRenderer.Page.RENDER_MODE_FOR_DISPLAY)
            page.close(); rnd.close(); pfd.close()
            bmp
    ...
    private fun renderVault() {
        ...
        val topLevel = (fileCats.keys + saved)
        ...
        if (topLevel.isEmpty()) {
            tvEmpty.text = "No folders — tap New Folder"
        ...
        topLevel.forEach { cat ->
            val allInCat = fileCats.entries
                .filter { it.key == cat || it.key.startsWith("$cat/") }
                .flatMap { it.value }
            ...
            if (expandedCategories.contains(cat)) {
                val subFolders = (fileCats.keys + saved).filter {
                    it.startsWith("$cat/") &&
                            it.removePrefix("$cat/").isNotEmpty() &&
                ...
                // FIX: "Add Subcategory" — uses showAddSubCat correctly
    ...
    private fun buildVaultCard(
        category: String, count: Int, depth: Int
    ): LinearLayout {
        ...
        return LinearLayout(this).apply {
            ...
            addView(TextView(this@MainActivity).apply {
                ...
                setTextColor(c_acc)
                background = GradientDrawable().apply {
                    setColor(Color.argb(26, 255, 214, 10))
            ...
            tc.addView(TextView(this@MainActivity).apply {
                text = displayName
                textSize = if (depth == 0) 14f else 12f
                typeface = Typeface.DEFAULT_BOLD
                setTextColor(Color.parseColor(txt1()))
            })
            tc.addView(TextView(this@MainActivity).apply {
    ...
    private fun showCategoryContextMenu(cat: String) {
        AlertDialog.Builder(this).setTitle(cat)
            .setItems(arrayOf("Add Sub-folder", "Rename", "Delete", "Move Files Here")) { _, w ->
                when (w) {
                    // FIX: "Add Subcategory" — passes correct parent
                    0 -> showAddSubCat(cat)
                    1 -> showRenameCat(cat)
                    2 -> confirmDeleteCat(cat)
                    3 -> showMoveAllDialog(cat)
                }
            }.show()
    }
    ...
    private fun showAddSubCat(parent: String) {
        ...
        AlertDialog.Builder(this)
            .setTitle("New sub-folder under \"${parent.substringAfterLast("/")}\"")
            .setView(et)
            .setPositiveButton("Add") { _, _ ->
                val n = et.text.toString().trim()
                if (n.isNotEmpty()) {
                    val s =
                        prefs.getStringSet("user_categories", mutableSetOf())!!
                            .toMutableSet()
                    s.add("$parent/$n")
                    prefs.edit().putStringSet("user_categories", s).apply()
                    // auto-expand the parent so the new sub-folder is visible
        ...
        AlertDialog.Builder(this).setTitle("Rename folder").setView(et)
            .setPositiveButton("Save") { _, _ ->
                ...
                if (n.isNotEmpty() && n != displayName) {
                    val prefix =
                        if (cat.contains("/")) cat.substringBeforeLast("/") + "/" else ""
                    ...
                    lifecycleScope.launch(Dispatchers.IO) {
                        allFileEntities
                            .filter {
                                it.category == cat || it.category.startsWith("$cat/")
                            }
    ...
    private fun confirmDeleteCat(cat: String) {
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Delete") { _, _ ->
                val s =
                    prefs.getStringSet("user_categories", mutableSetOf())!!.toMutableSet()
                s.removeAll { it == cat || it.startsWith("$cat/") }
                prefs.edit().putStringSet("user_categories", s).apply()
                lifecycleScope.launch(Dispatchers.IO) {
                    allFileEntities
                        .filter { it.category == cat || it.category.startsWith("$cat/") }
    ...
    private fun showMoveAllDialog(targetCat: String) {
        ...
        AlertDialog.Builder(this).setTitle("Move all files from:")
            .setItems(saved.toTypedArray()) { _, i ->
                ...
                lifecycleScope.launch(Dispatchers.IO) {
                    ...
                    withContext(Dispatchers.Main) {
                        rebuildFileList()
                        toast("Moved files to ${targetCat.substringAfterLast("/")}")
                    }
                }
            }.show()
    }
    ...
    private fun showMoveToCategoryDialog(f: RecentFileEntity) {
        val saved =
            (prefs.getStringSet("user_categories", emptySet()) ?: emptySet()).sorted()
    ...
    // ─────────────────────────────────────────────────────────
    // BOOKMARKS  (FIX: store Filename + Page Number, navigate on tap)
    // ─────────────────────────────────────────────────────────
    // Bookmark entries in SharedPreferences are stored as a StringSet where
    // each element = "pageIndex:label". The key is uri.hashCode().
    // The Bookmarks tab lists files that have at least one bookmark entry.
    // Clicking a bookmark in showBookmarksDialog() opens ViewerActivity and
    // passes the page number so the viewer scrolls there.
            ...
            .setItems(
                ...
                when (i) {
                    0 -> AlertDialog.Builder(this).setTitle("Clear Recent?")
                        .setMessage(
                            "Removes non-starred, non-categorized files from recent list only." +
                                    " Starred and categorized files are kept."
                        )
                        .setPositiveButton("Clear") { _, _ ->
                            // FIX: actually clears the entries via DAO
                            lifecycleScope.launch(Dispatchers.IO) {
                                db.recentFilesDao().clearRecentOnly()
                            }
                        }.setNegativeButton("Cancel", null).show()

                    1 -> AlertDialog.Builder(this).setTitle("Delete ALL?")
                        .setMessage("This removes every file from the list.")
    ...
    private fun showOcrDialog() {
        ...
        AlertDialog.Builder(this).setTitle("OCR — Text Recognition")
            .setItems(options) { _, w ->
                when (w) {
                    0 -> startActivity(
                        Intent(this, DocumentScannerActivity::class.java)
                    )
                    ...
                    3 -> {
                        ...
                        if (last != null) openUri(Uri.parse(last.uri))
    ...
    private fun showFileInfo(f: RecentFileEntity) {
        val bmPrefs = getSharedPreferences("propdf_bookmarks", MODE_PRIVATE)
        val bms =
            (bmPrefs.getStringSet(f.uri.hashCode().toString(), emptySet()) ?: emptySet()).size
        val info =
