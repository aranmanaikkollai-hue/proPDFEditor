// PATH: app/src/main/java/com/propdf/editor/ui/MainActivity.kt
...
class MainActivity : AppCompatActivity() {
    ...
    // Recent files list views
    private lateinit var listContainer : LinearLayout
    private lateinit var tvEmpty       : TextView
    private lateinit var scrollRecent  : ScrollView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Handle PDF shared from another app
        if (intent?.action == Intent.ACTION_VIEW && intent.data != null) {
            saveRecentAndOpen(intent.data!!)
        }

        // Observe recent files
    ...
    private fun buildRecentUI() {
        val root = findViewById<ViewGroup>(android.R.id.content)
        // Recent list is built programmatically into the main layout
        // The actual layout is activity_main.xml with scrollview + container
    }
    ...
    private fun refreshRecentList(files: List<RecentFileEntity>) {
        ...
        files.forEach { f ->
            ...
            val icon = TextView(this).apply {
                text = "📄"; textSize = 28f
                layoutParams = LinearLayout.LayoutParams(-2, -2).apply { setMargins(0, 0, dp(12), 0) }
            }
            ...
            info.addView(TextView(this).apply {
                ...
                textSize = 11f; setTextColor(android.graphics.Color.parseColor("#888888"))
            })
            ...
            row.addView(icon); row.addView(info); row.addView(btnDelete)
    ...
    private fun showSettings() {
        ...
        android.app.AlertDialog.Builder(this)
            .setTitle("⚙️ Settings")
            .setItems(arrayOf(
                if (isDark) "☀️  Light Mode" else "🌙  Dark Mode",
                "🗑️  Clear recent files",
                "ℹ️  About ProPDF Editor v3.0"
            )) { _, which ->
                when (which) {
                    0 -> {
                    ...
                    2 -> Toast.makeText(this,
                        "ProPDF Editor v3.0 — Free, No Ads, All Premium Features",
