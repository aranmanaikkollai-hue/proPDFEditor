...
class MainActivity : AppCompatActivity() {
    ...
    private fun buildSectionRow(): LinearLayout {
        return LinearLayout(this).apply {
            orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER_VERTICAL
            setPadding(dp(20), dp(16), dp(20), dp(8)); setBackgroundColor(bg())
            tvSection = TextView(this@MainActivity).apply {
                text = if (isDark) "Recent Files" else "LAST ACCESSED DOCUMENTS"
                textSize = if (isDark) 22f else 9f; typeface = Typeface.DEFAULT_BOLD
    ...
    private fun showSortMenu() {
        ...
        AlertDialog.Builder(this).setTitle("Sort files by").setItems(options) { _, which ->
            when (which) {
    ...
    // FIX: uses catDetailName instead of tvSection.text
    ...
    private fun rebuildFileList() {
        ...
        if (::tvSection.isInitialized) {
            tvSection.text = when {
                ...
                isDark  && currentTab == "cat_detail" -> catDetailName
                !isDark && currentTab == "recent"     -> "LAST ACCESSED DOCUMENTS"
                !isDark && currentTab == "starred"    -> "STARRED DOCUMENTS"
                !isDark && currentTab == "categories" -> "MY VAULT"
                !isDark && currentTab == "cat_detail" -> catDetailName.uppercase()
    ...
    // FIX: Grid card with real PDF thumbnail + setMargins (not margin =)
    private fun buildFileCardCompact(entity: RecentFileEntity): LinearLayout {
        ...
        // FIX: use setMargins() not margin = (which doesn't exist on FrameLayout.LayoutParams)
        thumbFrame.addView(TextView(this).apply {
            ...
            setTextColor(Color.WHITE); setPadding(dp(4), dp(2), dp(4), dp(2))
        ...
        if (cached != null) {
            thumbIv.setImageBitmap(cached); letterTv.visibility = View.GONE
        } else {
            lifecycleScope.launch(Dispatchers.IO) {
                ...
                withContext(Dispatchers.Main) {
                    if (bmp != null) {
                        ...
                        thumbIv.setImageBitmap(bmp); letterTv.visibility = View.GONE
                    }
                }
            }
        }
    ...
    private fun renderVault() {
        ...
        if (names.isEmpty()) {
            tvEmpty.text = "No folders yet"; tvEmpty.visibility = View.VISIBLE; return
        }
    ...
    // FIX: shows first letter of folder name, not hardcoded "F"
    private fun buildVaultCard(category: String, count: Int): LinearLayout {
        ...
        return LinearLayout(this).apply {
            ...
            addView(TextView(this@MainActivity).apply {
                ...
                text = folderLetter; gravity = Gravity.CENTER; textSize = 16f
                typeface = Typeface.DEFAULT_BOLD; setTextColor(c_acc)
                background = GradientDrawable().apply {
            ...
            addView(textCol)
            addView(TextView(this@MainActivity).apply {
                text = chevron; textSize = 14f; setTextColor(Color.parseColor(txt2()))
            })
            setOnClickListener {
    ...
    private fun showAddSubCategoryDialog(parent: String) {
        ...
        AlertDialog.Builder(this).setTitle("New sub-category under \"$parent\"").setView(input)
            .setPositiveButton("Add") { _, _ ->
                ...
                if (name.isNotEmpty()) {
                    ...
                    rebuildFileList(); toast("Added: $parent/$name")
                }
                ...
                if (newName.isNotEmpty() && newName != category) {
                    ...
                    val updated = saved.map { entry ->
                        when {
                            ...
                            entry.startsWith("$category/") -> newName + entry.substring(category.length)
                            else -> entry
                        }
                    }.toMutableSet()
    ...
    private fun confirmDeleteCategory(category: String) {
        AlertDialog.Builder(this).setTitle("Delete \"$category\"?").setMessage("Files will move to General.")
            .setPositiveButton("Delete") { _, _ ->
                val saved = prefs.getStringSet("user_categories", mutableSetOf())!!.toMutableSet()
                saved.removeAll { it == category || it.startsWith("$category/") }
                prefs.edit().putStringSet("user_categories", saved).apply(); rebuildFileList()
            }.setNegativeButton("Cancel", null).show()
    }
        ...
        AlertDialog.Builder(this).setTitle("New Folder").setView(et)
            .setPositiveButton("Create") { _, _ ->
                ...
                if (name.isNotEmpty()) {
                    ...
                    ex.add(name); prefs.edit().putStringSet("user_categories", ex).apply(); rebuildFileList()
                }
            }.show()
    }
    ...
    private fun showMoveToCategoryDialog(f: RecentFileEntity) {
        ...
        if (saved.isEmpty()) { toast("No folders yet. Create one in Vault tab."); return }
        ...
        AlertDialog.Builder(this).setTitle("OCR - Text Recognition")
            .setItems(options) { _, which ->
                when (which) {
                    ...
                    1 -> ocrImagePicker.launch("image/*")
                    2 -> toast("Open a PDF first, then use viewer OCR menu")
                    3 -> toast("Select multiple PDFs from Files tab")
                    4 -> {
                        val last = allFileEntities.maxByOrNull { it.lastOpenedAt }
                        if (last != null) openUri(Uri.parse(last.uri))
                        else toast("No recent files")
                    }
                    5 -> toast("Open a PDF, OCR it, then use Translate option")
                }
            }.setNegativeButton("Cancel", null).show()
    }
