...
class MainActivity : AppCompatActivity() {
    ...
    private fun buildUI() {
        ...
        val column = LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL; layoutParams = FrameLayout.LayoutParams(-1, -1)
        }
    ...
    private fun buildHeader(): LinearLayout {
        return LinearLayout(this).apply {
            ...
            setBackgroundColor(bg()); setPadding(dp(16), dp(4), dp(12), dp(4))
            addView(FrameLayout(this@MainActivity).apply {
                ...
                addView(ImageView(this@MainActivity).apply {
                    setImageResource(android.R.drawable.ic_menu_agenda); setColorFilter(Color.WHITE)
            ...
            addView(ImageButton(this@MainActivity).apply {
                ...
                setPadding(dp(10), dp(10), dp(10), dp(10)); layoutParams = LinearLayout.LayoutParams(dp(44), dp(44))
    ...
    // -------------------------------------------------------
    // TAB BAR  (Recent | Starred | Categories | Bookmarks)
    // -------------------------------------------------------
    ...
    private fun buildTabBar(): LinearLayout {
        ...
        tabRow = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER_VERTICAL; setPadding(dp(8), dp(8), dp(8), dp(4)) }
        // 4 tabs including Bookmarks
        listOf("Recent" to "recent", "Starred" to "starred", "Vault" to "categories", "Bookmarks" to "bookmarks").forEach { (label, id) ->
            ...
            tabRow.addView(TextView(this).apply {
                ...
                if (isDark && active) {
                    ...
                    outlineProvider = object : ViewOutlineProvider() { override fun getOutline(v: View, o: android.graphics.Outline) { o.setRoundRect(0, 0, v.width, v.height, dp(20).toFloat()) } }
                    clipToOutline = true
                }
        ...
        wrapper.addView(buildSortViewRow())
    ...
    private fun buildSortViewRow(): LinearLayout {
        return LinearLayout(this).apply {
            ...
            viewToggleBtn = ImageButton(this@MainActivity).apply {
                ...
                setImageResource(viewToggleIcon()); colorFilter = PorterDuffColorFilter(c_pri, PorterDuff.Mode.SRC_IN)
    ...
    private fun buildSectionRow(): LinearLayout {
        return LinearLayout(this).apply {
            ...
            tvSection = TextView(this@MainActivity).apply {
                ...
                textSize = if (isDark) 20f else 9f; typeface = Typeface.DEFAULT_BOLD
    ...
    private fun buildBottomNav(): FrameLayout {
        ...
        frame.addView(View(this).apply {
            layoutParams = FrameLayout.LayoutParams(-1, dp(1)).apply { gravity = Gravity.BOTTOM; bottomMargin = dp(68) }
        ...
        val navBar = LinearLayout(this).apply {
            ...
            setBackgroundColor(navBg()); setPadding(0, 0, 0, dp(8))
        }
        ...
        listOf(
            ...
            NI("ocr",     "OCR",   android.R.drawable.ic_menu_edit)
        ).forEach { item ->
            if (item.id == "__fab__") {
                navBar.addView(View(this).apply { layoutParams = LinearLayout.LayoutParams(0, -2, 1f) })
                return@forEach
            }
            ...
            col.addView(ImageView(this).apply {
                setImageResource(item.icon); setColorFilter(if (active) c_pri else Color.parseColor(txt2()))
                layoutParams = LinearLayout.LayoutParams(dp(22), dp(22)); tag = "${item.id}_icon"
            })
            col.addView(TextView(this).apply {
                ...
                setPadding(0, dp(2), 0, 0); tag = "${item.id}_label"
            })
            navBar.addView(col)
        }
        val fab = FrameLayout(this).apply {
            val s = dp(56); layoutParams = FrameLayout.LayoutParams(s, s).apply {
                gravity = Gravity.BOTTOM or Gravity.CENTER_HORIZONTAL; bottomMargin = dp(22)
            }
            setBackgroundColor(c_pri)
            ...
            clipToOutline = true; elevation = dp(10).toFloat()
        ...
        frame.addView(navBar); frame.addView(fab)
        return frame
    }
    ...
    private fun toggleTheme() {
        isDark = !isDark; prefs.edit().putBoolean("dark_mode", isDark).apply()
        thumbnailCache.clear(); buildUI(); observeFiles()
    }

    private fun observeFiles() {
        lifecycleScope.launch {
            db.recentFilesDao().getAll().collect { files -> allFileEntities = files; rebuildFileList() }
        }
    }

    // -------------------------------------------------------
    // SORT  (actually works: sorts allFileEntities before display)
    // -------------------------------------------------------
    ...
    private fun viewToggleIcon() = when (viewMode) {
    ...
    private fun showSortMenu() {
        val options = arrayOf("Date (newest)","Date (oldest)","Name A-Z","Name Z-A","Size (large)","Size (small)")
        AlertDialog.Builder(this).setTitle("Sort by").setItems(options) { _, which ->
            when (which) {
    ...
    private fun getBookmarkedFiles(): List<RecentFileEntity> {
        val prefs2 = getSharedPreferences("propdf_bookmarks", MODE_PRIVATE)
        return allFileEntities.filter { entity ->
            val key = entity.uri.hashCode().toString()
            val bms = prefs2.getStringSet(key, emptySet()) ?: emptySet()
            bms.isNotEmpty()
        }
    }
    ...
    private fun rebuildFileList() {
        ...
        // Sort/view controls always visible (inside vault too -- handled below)
        if (::tvSection.isInitialized) {
            tvSection.text = when (currentTab) {
                "recent"     -> if (isDark) "Recent Files" else "LAST ACCESSED"
                "starred"    -> if (isDark) "Starred Files" else "STARRED"
                "categories" -> if (isDark) "My Vault" else "MY VAULT"
                "bookmarks"  -> "Bookmarks"
                "cat_detail" -> catDetailName
        ...
        tvEmpty.text = when (currentTab) {
            "starred"   -> "No starred files\nTap the star on any file to star it"
            "bookmarks" -> "No bookmarks\nOpen a PDF and bookmark pages from the viewer"
            else        -> "No files yet\nTap + to open a PDF"
        }
        when (viewMode) {
    ...
    private fun buildFileCard(f: RecentFileEntity): View {
        ...
        thumb.addView(TextView(this).apply {
            text = f.displayName.take(1).uppercase(); textSize = if (isDark) 20f else 8f; typeface = Typeface.DEFAULT_BOLD
            setTextColor(if (isDark) c_pri else Color.parseColor("#E53935")); gravity = Gravity.CENTER
    ...
    private fun buildFileCardCompact(entity: RecentFileEntity): LinearLayout {
        ...
        val card = LinearLayout(this).apply {
            ...
            background = GradientDrawable().apply { setColor(C_CARD); cornerRadius = dp(12).toFloat() }
        ...
        val letterTv = TextView(this).apply { layoutParams = FrameLayout.LayoutParams(-2, -2, Gravity.CENTER); text = initial; textSize = 28f; typeface = Typeface.DEFAULT_BOLD; setTextColor(Color.parseColor("#448AFF")) }
        thumbFrame.addView(thumbIv); thumbFrame.addView(letterTv)
        thumbFrame.addView(TextView(this).apply {
            layoutParams = FrameLayout.LayoutParams(-2, -2, Gravity.BOTTOM or Gravity.END).apply { setMargins(0, 0, dp(4), dp(4)) }
            text = "PDF"; textSize = 8f; typeface = Typeface.DEFAULT_BOLD; setTextColor(Color.WHITE)
            setPadding(dp(4), dp(2), dp(4), dp(2))
            background = GradientDrawable().apply { setColor(Color.parseColor("#E53935")); cornerRadius = dp(4).toFloat() }
        })
        card.addView(thumbFrame)
        card.addView(TextView(this).apply {
            layoutParams = LinearLayout.LayoutParams(-1, -2).apply { topMargin = dp(5) }
            text = entity.displayName.removeSuffix(".pdf"); textSize = 11f; typeface = Typeface.DEFAULT_BOLD
            setTextColor(Color.parseColor(txt1())); maxLines = 1; ellipsize = android.text.TextUtils.TruncateAt.END
        })
        ...
        if (cached != null) { thumbIv.setImageBitmap(cached); letterTv.visibility = View.GONE }
        else lifecycleScope.launch(Dispatchers.IO) {
            val bmp = renderFirstPage(entity.uri)
            withContext(Dispatchers.Main) {
                if (bmp != null) { thumbnailCache[entity.uri] = bmp; thumbIv.setImageBitmap(bmp); letterTv.visibility = View.GONE }
            }
        }
        return card
    }
    ...
    private fun buildFileTileRow(entity: RecentFileEntity): LinearLayout {
        return LinearLayout(this).apply {
            ...
            addView(TextView(this@MainActivity).apply {
                layoutParams = LinearLayout.LayoutParams(dp(30), dp(30)); text = "PDF"
                gravity = Gravity.CENTER; textSize = 8f; typeface = Typeface.DEFAULT_BOLD; setTextColor(Color.WHITE)
                background = GradientDrawable().apply { setColor(Color.parseColor(DANGER)); cornerRadius = dp(4).toFloat() }
            })
            addView(TextView(this@MainActivity).apply {
            ...
            addView(TextView(this@MainActivity).apply {
                text = "${formatSize(entity.fileSizeBytes)} \u2022 ${relativeTime(entity.lastOpenedAt)}"; textSize = 10f
    ...
    private fun renderVault() {
        // Sort/view controls inside vault too
        ...
        if (names.isEmpty()) { tvEmpty.text = "No folders -- tap New Folder"; tvEmpty.visibility = View.VISIBLE; return }
        tvEmpty.visibility = View.GONE
        names.forEach { cat ->
            fileListContainer.addView(buildVaultCard(cat, fileCats[cat]?.size ?: 0))
            if (expandedCategories.contains(cat)) {
                // Sort applies inside category too
                val catFiles = sortedFilesForCat(fileCats[cat] ?: emptyList())
                catFiles.forEach { f -> fileListContainer.addView(buildFileTileRow(f).also { row -> (row.layoutParams as? LinearLayout.LayoutParams)?.marginStart = dp(20) }) }
            }
        }
    }
    ...
    private fun sortedFilesForCat(files: List<RecentFileEntity>): List<RecentFileEntity> {
        val sorted = when (sortMode) {
            "name" -> files.sortedBy { it.displayName.lowercase(Locale.getDefault()) }
            "size" -> files.sortedBy { it.fileSizeBytes }
    ...
    private fun buildVaultCard(category: String, count: Int): LinearLayout {
        ...
        return LinearLayout(this).apply {
            ...
            elevation = dp(2).toFloat()
            addView(TextView(this@MainActivity).apply {
                layoutParams = LinearLayout.LayoutParams(dp(42), dp(42)); text = folderLetter
                gravity = Gravity.CENTER; textSize = 16f; typeface = Typeface.DEFAULT_BOLD; setTextColor(c_acc)
                background = GradientDrawable().apply { setColor(Color.argb(26, 255, 214, 10)); cornerRadius = dp(8).toFloat() }
            })
            val tc = LinearLayout(this@MainActivity).apply { orientation = LinearLayout.VERTICAL; layoutParams = LinearLayout.LayoutParams(0, -2, 1f).apply { marginStart = dp(12) } }
            tc.addView(TextView(this@MainActivity).apply { text = category; textSize = 14f; typeface = Typeface.DEFAULT_BOLD; setTextColor(Color.parseColor(txt1())) })
    ...
    private fun showCategoryContextMenu(cat: String) {
        AlertDialog.Builder(this).setTitle(cat).setItems(arrayOf("Add Sub-category","Rename","Delete")) { _, w ->
            when (w) { 0 -> showAddSubCat(cat); 1 -> showRenameCat(cat); 2 -> confirmDeleteCat(cat) }
        }.show()
    }
    private fun showAddSubCat(parent: String) {
        val et = EditText(this).apply { hint = "Sub-category name" }
        AlertDialog.Builder(this).setTitle("Sub-category under \"$parent\"").setView(et)
    ...
    private fun showRenameCat(cat: String) {
        val et = EditText(this).apply { setText(cat) }
        AlertDialog.Builder(this).setTitle("Rename").setView(et)
            .setPositiveButton("Save") { _, _ ->
                val n = et.text.toString().trim()
                if (n.isNotEmpty() && n != cat) {
                    val s = prefs.getStringSet("user_categories", mutableSetOf())!!.toMutableSet()
                    val updated = s.map { if (it == cat) n else if (it.startsWith("$cat/")) n + it.substring(cat.length) else it }.toMutableSet()
                    prefs.edit().putStringSet("user_categories", updated).apply()
                    lifecycleScope.launch(Dispatchers.IO) { allFileEntities.filter { it.category == cat }.forEach { db.recentFilesDao().setCategory(it.uri, n) }; withContext(Dispatchers.Main) { rebuildFileList() } }
                }
            }.setNegativeButton("Cancel", null).show()
    }
    private fun confirmDeleteCat(cat: String) {
        AlertDialog.Builder(this).setTitle("Delete \"$cat\"?").setMessage("Files will move to General.")
            .setPositiveButton("Delete") { _, _ ->
                val s = prefs.getStringSet("user_categories", mutableSetOf())!!.toMutableSet()
                s.removeAll { it == cat || it.startsWith("$cat/") }; prefs.edit().putStringSet("user_categories", s).apply(); rebuildFileList()
            }.setNegativeButton("Cancel", null).show()
    }
    ...
    private fun showMoveToCategoryDialog(f: RecentFileEntity) {
        val saved = (prefs.getStringSet("user_categories", emptySet()) ?: emptySet()).toList().sorted()
        if (saved.isEmpty()) { toast("No folders -- create one in Vault tab"); return }
        AlertDialog.Builder(this).setTitle("Move to Folder").setItems(saved.toTypedArray()) { _, i ->
    ...
    private fun showFileOptions(f: RecentFileEntity) {
        val ops = arrayOf("Open", if (f.isFavourite) "Unstar" else "Star","Move to Category","Delete","Share")
        AlertDialog.Builder(this).setTitle(f.displayName).setItems(ops) { _, w ->
            when (w) {
                ...
                3 -> lifecycleScope.launch(Dispatchers.IO) { db.recentFilesDao().delete(f.uri) }
                4 -> shareFile(f)
            }
        }.show()
    }
    private fun showSearchDialog() {
        ...
        et.addTextChangedListener(object : android.text.TextWatcher {
            override fun afterTextChanged(s: android.text.Editable?) {
                ...
                fileListContainer.removeAllViews()
                allFileEntities.filter { it.displayName.lowercase().contains(q) }.forEach { fileListContainer.addView(buildFileCard(it)) }
            }
            override fun beforeTextChanged(s: CharSequence?, a: Int, b: Int, c: Int) {}
            override fun onTextChanged(s: CharSequence?, a: Int, b: Int, c: Int) {}
        })
        AlertDialog.Builder(this).setTitle("Search").setView(et).setNegativeButton("Close") { _, _ -> rebuildFileList() }.show()
    }
    private fun showSettings() {
        AlertDialog.Builder(this).setTitle("Settings")
            .setItems(arrayOf("Clear recent files","About","ProPDF v4.2 (Open Source)")) { _, i ->
                if (i == 0) lifecycleScope.launch(Dispatchers.IO) { db.recentFilesDao().clearAll() }
                else if (i == 1) toast("ProPDF Editor v4.2 -- built with Claude AI")
            }.show()
    }
    ...
    private fun showOcrDialog() {
        ...
        AlertDialog.Builder(this).setTitle("OCR -- Text Recognition").setItems(options) { _, w ->
            when (w) {
                ...
                2 -> toast("Open a PDF first, then use viewer OCR icon")
    ...
    private fun shareFile(f: RecentFileEntity) {
        try {
            ...
            startActivity(Intent.createChooser(Intent(Intent.ACTION_SEND).apply {
                type = "application/pdf"; putExtra(Intent.EXTRA_STREAM, uri2); addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
            }, "Share PDF"))
    ...
    private fun hdrBtn(icon: Int, action: () -> Unit) = ImageButton(this).apply {
        setImageResource(icon); setBackgroundColor(Color.TRANSPARENT); setColorFilter(Color.parseColor(txt2()))
