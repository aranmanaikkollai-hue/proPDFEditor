...
class MainActivity : AppCompatActivity() {
    ...
    private fun showMoveToCategoryDialog(f: RecentFileEntity, existingCats: List<String>) {
        // Merge DB categories + SharedPrefs saved categories + defaults
        val prefs    = getSharedPreferences("propdf_prefs", MODE_PRIVATE)
        val savedCats = prefs.getStringSet("user_categories", emptySet()) ?: emptySet()
        val fileCats  = allFiles.map { it.category }.filter { it.isNotEmpty() }
        val combined  = (fileCats + savedCats + existingCats +
                        listOf("General", "Work", "Personal", "Education"))
                        .distinct().filter { it.isNotEmpty() }.sorted()
        val allCats   = combined.toTypedArray()

        if (allCats.isEmpty()) {
            // No categories yet - go straight to create
            showCreateAndMoveCategoryDialog(f)
            return
        }
        ...
        android.app.AlertDialog.Builder(this)
            ...
            .setItems(allCats) { _, i ->
                val chosen = allCats[i]
                // Persist category in prefs so it stays even when empty
                val existing = prefs.getStringSet("user_categories", mutableSetOf()) ?: mutableSetOf()
    ...
    private fun showCreateAndMoveCategoryDialog(f: RecentFileEntity) {
        val et = EditText(this).apply {
            hint = "Category name (e.g. Work, Personal)"
        ...
        android.app.AlertDialog.Builder(this).setTitle("New Category").setView(et)
            .setPositiveButton("Create & Move") { _, _ ->
                ...
                toast("Created '$name' and moved file")
                refreshDisplay()
            }.setNegativeButton("Cancel", null).show()
    }
