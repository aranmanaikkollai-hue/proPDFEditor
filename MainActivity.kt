...
class MainActivity : AppCompatActivity() {
    ...
    private fun buildTabBar(): LinearLayout {
        ...
        listOf("Recent" to "recent", "Starred" to "starred", "Vault" to "categories")
            .forEach { (label, id) ->
                ...
                tabRow.addView(tv)
                if (id != "categories") {
                    tabRow.addView(View(this).apply { layoutParams = LinearLayout.LayoutParams(dp(4), dp(1)) })
                }
            }

        wrapper.addView(tabRow)
        wrapper.addView(buildSortViewControlRow()) // From Additions
    ...
    private fun buildSectionRow(): LinearLayout {
        return LinearLayout(this).apply {
            ...
            addView(TextView(this@MainActivity).apply {
                ...
                setTextColor(Color.parseColor(txt2())); letterSpacing = 0.1f
            })
        }
    }
    ...
    // ================================================================
    // INTEGRATED ADDITIONS: UI Helpers & Logic
    // ================================================================
    ...
    private fun sortedFiles(): List<RecentFileEntity> {
        val baseList = when (currentTab) {
            ...
            "cat_detail" -> allFileEntities.filter { it.category == tvSection.text }
            else         -> allFileEntities
        }
        val sorted = when (sortMode) {
            "name" -> baseList.sortedBy { it.displayName.lowercase() }
            "size" -> baseList.sortedBy { it.fileSizeBytes }
            else   -> baseList.sortedBy { it.lastOpenedAt }
        }
...
if (currentTab == "categories") {
    ...
    private fun buildFileCard(f: RecentFileEntity): View {
        ...
        val thumb = FrameLayout(this).apply {
            ...
            outlineProvider = object : ViewOutlineProvider() {
                override fun getOutline(v: View, o: android.graphics.Outline) { o.setRoundRect(0,0,v.width,v.height, dp(8).toFloat()) }
            }
            clipToOutline = true
        }
...
val info = LinearLayout(this).apply {
        ...
        info.addView(TextView(this).apply {
            ...
            setTextColor(Color.parseColor(txt1())); isSingleLine = true; ellipsize = android.text.TextUtils.TruncateAt.END
        })
        info.addView(TextView(this).apply {
    ...
    private fun buildFileCardCompact(entity: RecentFileEntity): LinearLayout {
        ...
        return LinearLayout(this).apply {
            ...
            addView(TextView(this@MainActivity).apply {
                layoutParams = LinearLayout.LayoutParams(dp(44), dp(44))
                text = initial; gravity = Gravity.CENTER; textSize = 18f; typeface = Typeface.DEFAULT_BOLD

setTextColor(Color.WHITE); background = GradientDrawable().apply { setColor(Color.parseColor(DANGER)); cornerRadius = dp(8).toFloat() }
            })
            addView(TextView(this@MainActivity).apply {
                layoutParams = LinearLayout.LayoutParams(-1, -2).apply { topMargin = dp(6) }
                text = entity.displayName; textSize = 11f; typeface = Typeface.DEFAULT_BOLD
                setTextColor(Color.parseColor(txt1())); maxLines = 2; ellipsize = android.text.TextUtils.TruncateAt.END
            })
            addView(TextView(this@MainActivity).apply { text = sizeMb; textSize = 10f; setTextColor(Color.parseColor(txt2())) })
            setOnClickListener { openUri(entity.uri) }
    ...
    private fun buildFileTileRow(entity: RecentFileEntity): LinearLayout {
        ...
        return LinearLayout(this).apply {
            ...
            background = GradientDrawable().apply { setColor(C_CARD); cornerRadius = dp(8).toFloat() }
            
            addView(TextView(this@MainActivity).apply {
                layoutParams = LinearLayout.LayoutParams(dp(28), dp(28))
                text = "P"; gravity = Gravity.CENTER; textSize = 10f; typeface = Typeface.DEFAULT_BOLD
                setTextColor(Color.WHITE); background = GradientDrawable().apply { setColor(Color.parseColor(DANGER)); cornerRadius = dp(4).toFloat() }
            })
            addView(TextView(this@MainActivity).apply {
            ...
            addView(TextView(this@MainActivity).apply {
                text = "$sizeMb • $timeStr"; textSize = 10f; setTextColor(Color.parseColor(txt2()))
...
private fun renderVault() {
        ...
        fileListContainer.addView(TextView(this).apply {
            ...
            setTextColor(c_pri); gravity = Gravity.END; setPadding(0, 0, dp(16), dp(8))
    ...
    private fun buildVaultCard(category: String, count: Int): LinearLayout {
        ...
        return LinearLayout(this).apply {
            ...
            addView(TextView(this@MainActivity).apply {
                ...
                text = "F"; gravity = Gravity.CENTER; textSize = 18f; typeface = Typeface.DEFAULT_BOLD
                setTextColor(c_acc); background = GradientDrawable().apply { setColor(Color.argb(26, 255, 214, 10)); cornerRadius = dp(8).toFloat() }
            })

            val textCol = LinearLayout(this@MainActivity).apply {
                orientation = LinearLayout.VERTICAL; layoutParams = LinearLayout.LayoutParams(0, -2, 1f).apply { marginStart = dp(12) }
            }
            textCol.addView(TextView(this@MainActivity).apply {
                text = category; textSize = 14f; typeface = Typeface.DEFAULT_BOLD; setTextColor(Color.parseColor(txt1()))
            })
            textCol.addView(TextView(this@MainActivity).apply {
                text = "$count DOCUMENTS"; textSize = 10f; typeface = Typeface.DEFAULT_BOLD; setTextColor(Color.parseColor(txt2()))
            })
            addView(textCol)
            addView(TextView(this@MainActivity).apply { text = chevron; textSize = 16f; setTextColor(Color.parseColor(txt2())) })

            setOnClickListener {
                currentTab = "cat_detail"
                tvSection.text = category
                rebuildFileList()
            }
...
sortBtn = TextView(this@MainActivity).apply {
    ...
    private fun showCategoryContextMenu(parentCategory: String) {
        ...
        AlertDialog.Builder(this).setTitle(parentCategory).setItems(ops) { _, which ->
            when (which) {
                ...
                1 -> showRenameCategoryDialog(parentCategory)
    ...
    private fun showAddSubCategoryDialog(parent: String) {
        ...
        AlertDialog.Builder(this).setTitle("New sub-category").setView(input)
            .setPositiveButton("Add") { _, _ ->
                val name = input.text.toString().trim()
                if (name.isNotEmpty()) {
                    val saved = prefs.getStringSet("user_categories", mutableSetOf())!!.toMutableSet()
                    saved.add("$parent/$name"); prefs.edit().putStringSet("user_categories", saved).apply()
                    rebuildFileList()
                }
            }.show()
    }
...
private fun confirmDeleteCategory(category: String) {
        AlertDialog.Builder(this).setTitle("Delete?").setMessage("Files will move to General.")
            .setPositiveButton("Delete") { _, _ ->
                val saved = prefs.getStringSet("user_categories", mutableSetOf())!!.toMutableSet()
                saved.remove(category); prefs.edit().putStringSet("user_categories", saved).apply()
                rebuildFileList()
            }.show()
    }

    private fun showFileOptions(f: RecentFileEntity) {
        val ops = arrayOf("Open", if (f.isFavourite) "Unstar" else "Star", "Move to Category", "Delete from list", "Share")
...
private fun showMoveToCategoryDialog(f: RecentFileEntity) {
    ...
    private fun showCreateFolderDialog() {
        ...
        AlertDialog.Builder(this).setTitle("New Folder").setView(et).setPositiveButton("Create") { _, _ ->
            val name = et.text.toString().trim()
            if (name.isNotEmpty()) {
                val ex = prefs.getStringSet("user_categories", mutableSetOf())!!.toMutableSet()
                ex.add(name); prefs.edit().putStringSet("user_categories", ex).apply()
                rebuildFileList()
            }
        }.show()
    }
...
private fun showSearchDialog() {
    ...
    private fun showSettings() {
        AlertDialog.Builder(this).setTitle("Settings").setItems(arrayOf("Clear recent files", "About")) { _, i ->
            if (i == 0) lifecycleScope.launch(Dispatchers.IO) { db.recentFilesDao().clearAll() }
            else toast("ProPDF v4.2")
        }.show()
    }
