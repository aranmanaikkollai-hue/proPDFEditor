...
class MainActivity : AppCompatActivity() {
    ...
    private fun showSortMenu() {
        AlertDialog.Builder(this).setTitle("Sort by")
            .setItems(arrayOf("Date (newest)","Date (oldest)","Name A-Z","Name Z-A","Size (large)","Size (small)")) { _, which ->
                when (which) { 0->{sortMode="date";sortAsc=false}; 1->{sortMode="date";sortAsc=true}; 2->{sortMode="name";sortAsc=true}; 3->{sortMode="name";sortAsc=false}; 4->{sortMode="size";sortAsc=false}; 5->{sortMode="size";sortAsc=true} }
    ...
    private fun sortedFiles(): List<RecentFileEntity> {
        ...
        val sorted = when (sortMode) { "name" -> base.sortedBy { it.displayName.lowercase(Locale.getDefault()) }; "size" -> base.sortedBy { it.fileSizeBytes }; else -> base.sortedByDescending { it.lastOpenedAt } }
    ...
    private fun sortedFilesForCat(files: List<RecentFileEntity>): List<RecentFileEntity> {
        val sorted = when (sortMode) { "name" -> files.sortedBy { it.displayName.lowercase(Locale.getDefault()) }; "size" -> files.sortedBy { it.fileSizeBytes }; else -> files.sortedByDescending { it.lastOpenedAt } }
        return if (sortAsc) sorted.reversed() else sorted
    }
        ...
        AlertDialog.Builder(this).setTitle("Settings")
            .setItems(arrayOf("Clear Recent Files (keep starred/categorized)", "Delete ALL Files", "About")) { _, i ->
                when (i) {
                    ...
                    0 -> AlertDialog.Builder(this).setTitle("Clear Recent?").setMessage("Removes non-starred, non-categorized files from recent list only. Starred and categorized files are kept.")
                            .setPositiveButton("Clear") { _, _ -> lifecycleScope.launch(Dispatchers.IO) { db.recentFilesDao().clearRecentOnly() } }
                            .setNegativeButton("Cancel", null).show()
