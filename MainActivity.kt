...
orientation = LinearLayout.VERTICAL; setPadding(0, 0, 0, dp(90))
}
tvEmpty = TextView(this).apply {
text = "No files yet\nTap + to open a PDF"; textSize = 14f; gravity = Gravity.CENTER
setTextColor(Color.parseColor(txt2()))
setPadding(dp(32), dp(64), dp(32), dp(64)); visibility = View.GONE
}
fileListContainer = LinearLayout(this).apply {
orientation = LinearLayout.VERTICAL; setPadding(dp(16), dp(4), dp(16), 0)
}
...
if (!isDark) window.decorView.systemUiVisibility = window.decorView.systemUiVisibility or
View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR
else window.decorView.systemUiVisibility = window.decorView.systemUiVisibility and
...
val tc = LinearLayout(this@MainActivity).apply {
orientation = LinearLayout.VERTICAL
layoutParams = LinearLayout.LayoutParams(0, -2, 1f)
}
tc.addView(TextView(this@MainActivity).apply {
...
"Bookmarks" to "bookmarks").forEach { (label, id) ->
...
layoutParams = LinearLayout.LayoutParams(-1, dp(1)); setBackgroundColor(divLine())
})
return wrapper
}
...
setImageResource(viewToggleIcon()); rebuildFileList()
}
}
...
text = if (isDark) "Recent Files" else "LAST ACCESSED"
...
currentTab in listOf("recent", "starred", "categories", "cat_detail", "bookmarks")
...
(tabRow.findViewWithTag<View>("${t}_tv") as? TextView)?.apply {
...
(rootFrame.findViewWithTag<View>("${id}_icon") as? ImageView)
?.setColorFilter(if (active) c_pri else Color.parseColor(txt2()))
(rootFrame.findViewWithTag<View>("${id}_label") as? TextView)
?.setTextColor(if (active) c_pri else Color.parseColor(txt2()))
}
}
...
.setItems(arrayOf(
"Date (newest)", "Date (oldest)", "Name A-Z",
"Name Z-A", "Size (large)", "Size (small)"
)) { _, which ->
when (which) {
...
"starred" -> "No starred files\nTap the star on any file"
"bookmarks" -> "No bookmarks yet\nBookmark pages inside the PDF viewer"
else -> "No files yet\nTap + to open a PDF"
}
when (viewMode) {
...
setBackgroundColor(
if (isDark) Color.parseColor("#3A3A3C") else Color.parseColor("#FFEBEE")
)
outlineProvider = object : ViewOutlineProvider() {
override fun getOutline(v: View, o: android.graphics.Outline) {
...
text = f.displayName.take(1).uppercase(); textSize = 20f; typeface = Typeface.DEFAULT_BOLD
...
text = f.displayName.removeSuffix(".pdf"); textSize = 14f; typeface = Typeface.DEFAULT_BOLD
setTextColor(Color.parseColor(txt1())); isSingleLine = true
...
setColor(Color.parseColor(card())); cornerRadius = dp(12).toFloat()
}
...
layoutParams = FrameLayout.LayoutParams(-1, -1); scaleType = ImageView.ScaleType.CENTER_CROP
}
val lv = TextView(this).apply {
layoutParams = FrameLayout.LayoutParams(-2, -2, Gravity.CENTER)
text = entity.displayName.firstOrNull()?.uppercase() ?: "P"
textSize = 28f; typeface = Typeface.DEFAULT_BOLD; setTextColor(Color.parseColor("#448AFF"))
}
tf.addView(iv); tf.addView(lv)
tf.addView(TextView(this).apply {
...
card.addView(TextView(this).apply {
text = formatSize(entity.fileSizeBytes); textSize = 9f; setTextColor(Color.parseColor(txt2()))
})
...
if (bmp != null) { thumbnailCache[entity.uri] = bmp; iv.setImageBitmap(bmp); lv.visibility = View.GONE }
}
}
return card
}
...
background = GradientDrawable().apply {
setColor(Color.parseColor(card())); cornerRadius = dp(8).toFloat()
}
addView(TextView(this@MainActivity).apply {
layoutParams = LinearLayout.LayoutParams(dp(30), dp(30))
text = "PDF"; gravity = Gravity.CENTER; textSize = 8f; typeface = Typeface.DEFAULT_BOLD
...
addView(TextView(this@MainActivity).apply {
text = "${formatSize(entity.fileSizeBytes)} \u2022 ${relativeTime(entity.lastOpenedAt)}"
textSize = 10f; setTextColor(Color.parseColor(txt2()))
...
val bmpW = 320
val bmpH = (page.height.toFloat() / page.width.toFloat() * bmpW).toInt().coerceAtLeast(1)
...
val topLevel = (fileCats.keys + saved).filter { it.isNotEmpty() && !it.contains("/") }
.distinct().sorted()
...
gravity = Gravity.END; setPadding(0, 0, dp(16), dp(8))
...
val allInCat = fileCats.entries.filter { it.key == cat || it.key.startsWith("$cat/") }
.flatMap { it.value }
fileListContainer.addView(buildVaultCard(cat, allInCat.size, 0))
if (expandedCategories.contains(cat)) {
val subFolders = (fileCats.keys + saved).filter {
it.startsWith("$cat/") && it.removePrefix("$cat/").isNotEmpty() &&
!it.removePrefix("$cat/").contains("/")
}.distinct().sorted()
...
sortedFilesForCat(subFiles).forEach { f ->
fileListContainer.addView(buildFileTileRow(f).also { row ->
(row.layoutParams as? LinearLayout.LayoutParams)?.marginStart = dp(40)
})
}
}
}
...
setTextColor(c_acc)
background = GradientDrawable().apply {
setColor(Color.argb(26, 255, 214, 10)); cornerRadius = dp(6).toFloat()
}
})
val tc = LinearLayout(this@MainActivity).apply {
orientation = LinearLayout.VERTICAL
...
tc.addView(TextView(this@MainActivity).apply {
text = displayName; textSize = if (depth == 0) 14f else 12f
typeface = Typeface.DEFAULT_BOLD; setTextColor(Color.parseColor(txt1()))
})
tc.addView(TextView(this@MainActivity).apply {
text = "$count DOCUMENTS"; textSize = 10f; typeface = Typeface.DEFAULT_BOLD
setTextColor(Color.parseColor(txt2()))
})
addView(tc)
addView(TextView(this@MainActivity).apply {
text = if (isExpanded) "\u25bc" else "\u25b6"
textSize = 13f; setTextColor(Color.parseColor(txt2()))
})
setOnClickListener {
if (isExpanded) expandedCategories.remove(category)
else { expandedCategories.add(category); catDetailName = category }
rebuildFileList()
}
...
AlertDialog.Builder(this).setTitle(cat)
.setItems(arrayOf("Add Sub-folder", "Rename", "Delete", "Move Files Here")) { _, w ->
when (w) {
0 -> showAddSubCat(cat); 1 -> showRenameCat(cat)
2 -> confirmDeleteCat(cat); 3 -> showMoveAllDialog(cat)
}
}.show()
}
...
.setTitle("New sub-folder under \"${parent.substringAfterLast("/")}\"").setView(et)
.setPositiveButton("Add") { _, _ ->
val n = et.text.toString().trim()
if (n.isNotEmpty()) {
val s = prefs.getStringSet("user_categories", mutableSetOf())!!.toMutableSet()
s.add("$parent/$n"); prefs.edit().putStringSet("user_categories", s).apply()
rebuildFileList()
}
...
val updated = s.map {
if (it == cat) newFull
else if (it.startsWith("$cat/")) newFull + it.substring(cat.length)
else it
}.toMutableSet()
prefs.edit().putStringSet("user_categories", updated).apply()
lifecycleScope.launch(Dispatchers.IO) {
allFileEntities.filter { it.category == cat || it.category.startsWith("$cat/") }
.forEach { db.recentFilesDao().setCategory(it.uri, newFull) }
...
AlertDialog.Builder(this)
.setTitle("Delete \"${cat.substringAfterLast("/")}\"?")
.setMessage("Files will move to General (Recent).")
.setPositiveButton("Delete") { _, _ ->
val s = prefs.getStringSet("user_categories", mutableSetOf())!!.toMutableSet()
s.removeAll { it == cat || it.startsWith("$cat/") }
prefs.edit().putStringSet("user_categories", s).apply()
lifecycleScope.launch(Dispatchers.IO) {
allFileEntities.filter { it.category == cat || it.category.startsWith("$cat/") }
.forEach { db.recentFilesDao().setCategory(it.uri, "") }
withContext(Dispatchers.Main) { rebuildFileList() }
}
}.setNegativeButton("Cancel", null).show()
}
...
val saved = (prefs.getStringSet("user_categories", emptySet()) ?: emptySet())
.filter { !it.startsWith(targetCat) }.sorted()
...
val src = saved[i]
lifecycleScope.launch(Dispatchers.IO) {
allFileEntities.filter { it.category == src }
.forEach { db.recentFilesDao().setCategory(it.uri, targetCat) }
withContext(Dispatchers.Main) {
rebuildFileList(); toast("Moved files to ${targetCat.substringAfterLast("/")}")
}
}
}.show()
}
...
if (n.isNotEmpty()) {
val s = prefs.getStringSet("user_categories", mutableSetOf())!!.toMutableSet()
s.add(n); prefs.edit().putStringSet("user_categories", s).apply(); rebuildFileList()
}
}.show()
}
...
val ops = arrayOf(
"Open", if (f.isFavourite) "Unstar" else "Star",
"Move to Category", "Rename", "Delete", "Share", "File Info"
)
...
.setMessage("Remove \"${f.displayName}\" from recent list?")
...
lifecycleScope.launch(Dispatchers.IO) {
db.recentFilesDao().insert(f.copy(displayName = newName))
withContext(Dispatchers.Main) { toast("Renamed to $newName") }
}
}
}.setNegativeButton("Cancel", null).show()
}
...
AlertDialog.Builder(this).setTitle("File Info").setMessage(info)
...
if (results.isEmpty()) {
tvEmpty.text = "No files matching \"$q\""; tvEmpty.visibility = View.VISIBLE
} else {
tvEmpty.visibility = View.GONE
...
AlertDialog.Builder(this).setTitle("Search Files").setView(et)
...
.setItems(arrayOf(
"Clear Recent Files (keep starred/categorized)",
"Delete ALL Files", "About"
)) { _, i ->
when (i) {
0 -> AlertDialog.Builder(this).setTitle("Clear Recent?")
.setMessage("Removes non-starred, non-categorized files from recent list only. " +
"Starred and categorized files are kept.")
.setPositiveButton("Clear") { _, _ ->
lifecycleScope.launch(Dispatchers.IO) { db.recentFilesDao().clearRecentOnly() }
}.setNegativeButton("Cancel", null).show()
1 -> AlertDialog.Builder(this).setTitle("Delete ALL?")
.setMessage("This removes every file from the list including starred and categorized.")
.setPositiveButton("Delete All") { _, _ ->
lifecycleScope.launch(Dispatchers.IO) { db.recentFilesDao().clearAll() }
}.setNegativeButton("Cancel", null).show()
2 -> AlertDialog.Builder(this).setTitle("ProPDF Editor v4.2")
.setMessage("Production-grade PDF editor.\nAnnotation engine: data-driven.\n" +
...
val options = arrayOf(
"OCR from Camera (Scan)", "OCR from Gallery Image",
"OCR from open PDF", "Extract text from last opened",
"Open Tools (Merge/Split/Compress)"
)
...
if (last != null) openUri(Uri.parse(last.uri)) else toast("No recent files")
}
