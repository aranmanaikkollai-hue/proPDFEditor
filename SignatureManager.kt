...
// app/src/main/java/com/propdf/editor/data/repository/SignatureManager.kt
//
// FEATURE: Digital signature library
//   - Save drawn Bitmap to SharedPreferences as Base64 PNG
//   - Load stored signature back as Bitmap
//   - Insert signature Bitmap into PDF at (x, y) via iText PdfCanvas
//   - Works within existing TextAnnot annotation system
//
// RULES OBEYED:
//   - PdfDocument NOT Closeable: try{} finally{doc.close()}   (rule #1)
//   - PdfCanvas constructor via page.newContentStreamAfter()  (rule #3)
//   - No addImageAt() -- uses saveState+concatMatrix+addXObjectAt (rule #4/#5)
//   - Pure ASCII                                              (rule #32)
...
class SignatureManager @Inject constructor(
    ...
    suspend fun saveSignature(bmp: Bitmap, slot: String = "default"): Boolean =
    ...
    suspend fun loadSignature(slot: String = "default"): Bitmap? =
        withContext(Dispatchers.Default) {
            try {
                val b64 = prefs.getString("sig_$slot", null) ?: return@withContext null
    ...
    // x, y: bottom-left corner in PDF user-space points (origin = bottom-left)
    ...
    suspend fun insertSignatureIntoPdf(
        ...
        runCatching {
            ...
            try {
                ...
                try {
                    canvas.saveState()
                    // concatMatrix(scaleX, skewB, skewC, scaleY, transX, transY) -- rule #4
                    canvas.concatMatrix(
    ...
    // -----------------------------------------------------------------------
    // pageScale is screenPixels/pdfPoints (stored in ViewerActivity.pageScales map)
    ...
    fun screenToPdfCoords(
        ...
        val pdfY    = pdfPageHeightPt - (screenY / pageScale) - (sigHeightPx / pageScale)
