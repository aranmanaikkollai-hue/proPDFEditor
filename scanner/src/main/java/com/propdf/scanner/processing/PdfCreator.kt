...
class PdfCreator(private val context: Context) {
    ...
    private fun getPageSize(pageSize: PageSize, bitmap: Bitmap) = when (pageSize) {
        PageSize.A4 -> PageSize.A4
        PageSize.LETTER -> PageSize.LETTER
        PageSize.LEGAL -> PageSize.LEGAL
        PageSize.AUTO -> if (bitmap.width.toFloat() / bitmap.height > 1.0) PageSize.A4.rotate() else PageSize.A4
    }
