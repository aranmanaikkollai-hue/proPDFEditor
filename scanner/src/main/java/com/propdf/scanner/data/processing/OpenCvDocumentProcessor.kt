 ...
 */
class OpenCvDocumentProcessor @Inject constructor() {
    ...
    suspend fun detectDocumentEdges(bitmap: Bitmap): EdgeDetectionResult = withContext(Dispatchers.Default) {
        ...
        val noEdgesFallback = EdgeDetectionResult(
            listOf(
                PointF(0f, 0f),
        ...
        // This function runs on every analyzed camera frame during live
        // scanning (see ScannerViewModel.onAnalyzedFrame), several times a
        // second -- a Firebase Test Lab Robo crawl on a real device showed
        // it crashing on UnsatisfiedLinkError the first time a frame
        // reached native OpenCV. OpenCvAvailability.runSafely is the
        // circuit breaker: if OpenCV is (or becomes) unavailable, this
        // returns noEdgesFallback immediately without touching native code
        // again, instead of repeating the same broken call on every
        // subsequent frame.
        OpenCvAvailability.runSafely(TAG, noEdgesFallback) {
            var srcMat: Mat? = null
    ...
    suspend fun applyPerspectiveCorrection(
        ...
        OpenCvAvailability.runSafely(TAG, bitmap) {
            var srcMat: Mat? = null
