...
class OpenCvDocumentProcessor @Inject constructor() {
    ...
    suspend fun detectDocumentEdges(bitmap: Bitmap): EdgeDetectionResult = withContext(Dispatchers.Default) {
        ...
        try {
            ...
            // UnsatisfiedLinkError is an Error, not an Exception, so the
            // catch above never sees it -- this is exactly the crash a
            // Firebase Test Lab Robo run hit on a real device (native
            // OpenCV symbols unavailable), and this function runs on every
            // analyzed camera frame during live scanning, so left uncaught
            // it would crash-loop rather than crash once. ProPDFApplication
            // now explicitly loads the native library at startup, but this
            // still degrades to "no edges detected" instead of crashing if
            // that load ever genuinely fails on a given device/ABI.
            Log.e(TAG, "OpenCV native library unavailable -- edge detection disabled for this session", e)
            EdgeDetectionResult(
                listOf(
                    PointF(0f, 0f),
                    PointF(bitmap.width.toFloat(), 0f),
