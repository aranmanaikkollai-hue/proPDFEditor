...
class AutoRenameDocumentUseCase @Inject constructor(
    ...
    suspend operator fun invoke(params: Params): AppResult<Result> = withContext(dispatchers.io) {
        try {
            ...
            val shouldApply = detection.confidence >= 0.4f &&
                detection.type != DocumentTypeDetector.DocumentType.UNKNOWN
    ...
    private suspend fun applyRename(uri: Uri, newName: String): Boolean =
