    ...
    fun downloadModel(language: OcrLanguage) {
        viewModelScope.launch {
            downloadOcrModelUseCase(language).collect { result ->
                when (result) {
                    is AppResult.Success -> _uiState.value = OcrUiState.ModelDownloaded(language)
                    is AppResult.Error -> _uiState.value = OcrUiState.Error(result.message ?: "Download failed")
                    is AppResult.Loading -> _uiState.value = OcrUiState.Processing("Downloading model... ${result.data}%")
                }
            }
        }
    }
