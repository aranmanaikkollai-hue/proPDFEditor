...
class ScannerProcessor {
    ...
    suspend fun processDocument(bitmap: Bitmap, quality: ProcessQuality = ProcessQuality.MEDIUM): Bitmap? = withContext(Dispatchers.Default) {
        ...
        try {
            ...
            val safeWarped = warped!!

            // Apply quality enhancement
            enhance(safeWarped, quality)

            // Convert back to bitmap
            val result = Bitmap.createBitmap(safeWarped.cols(), safeWarped.rows(), Bitmap.Config.ARGB_8888)
            Utils.matToBitmap(safeWarped, result)
            result
