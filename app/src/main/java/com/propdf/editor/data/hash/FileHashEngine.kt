...
class FileHashEngine @Inject constructor(
    ...
    suspend fun computeFastHash(uri: Uri, fileSize: Long): String = withContext(dispatchers.io) {
        ...
        inputStream.use { stream ->
            ...
            while (remaining > 0 && stream.read(buffer, 0, minOf(buffer.size, remaining)).also { read = it } != -1) {
    ...
    suspend fun computeBothHashes(uri: Uri, fileSize: Long): Pair<String, String?> = withContext(dispatchers.io) {
