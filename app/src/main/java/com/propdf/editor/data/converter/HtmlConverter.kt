...
class HtmlConverter @Inject constructor(
    ...
    suspend fun toPdf(
        ...
        try {
            ...
            ConversionResult(false, null, fileName, e.toSafeUserMessage("This HTML file could not be converted to PDF."))
        }
    }
            ...
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                view?.postDelayed({
                    try {
                            ...
                            override fun onWrite(
                                ...
                                progressScope.launch(Dispatchers.IO) {
                                    try {
                                        ...
                                        continuation.resume(
                                            ConversionResult(false, null, outputFile.nameWithoutExtension, e.toSafeUserMessage("This document could not be saved."))
                                        )
                                    } finally {
                                        webView.destroy()
                                    }
                        ...
                        continuation.resume(
                            ConversionResult(false, null, outputFile.nameWithoutExtension, e.toSafeUserMessage("This document could not be printed."))
                        )
                        webView.destroy()
                    }
                }, 1000)
