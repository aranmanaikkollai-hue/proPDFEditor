...
class OcrIndexingEngine @Inject constructor(
    ...
    private val processingSemaphore = Semaphore(permits = 1)
    ...
    private suspend fun processPage(renderer: PdfRenderer, pageIndex: Int): String {
        ...
        return try {
            ...
            val appResult = ocrRepository.recognizeImage(bitmap, OcrConfig())
            bitmapPool.release(bitmap)
            when (appResult) {
                is AppResult.Success -> appResult.data.fullText
                else -> ""
            }
        } finally {
            page.close()
