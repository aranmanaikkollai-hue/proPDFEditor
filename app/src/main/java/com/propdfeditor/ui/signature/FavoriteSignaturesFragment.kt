...
import com.propdfeditor.databinding.FragmentSignatureListBinding
import com.propdfeditor.security.SignatureViewModel
import dagger.hilt.android.AndroidEntryPoint
import kotlinx.coroutines.launch

@AndroidEntryPoint
class FavoriteSignaturesFragment : Fragment() {
    ...
    private fun setupRecyclerView() {
        ...
        binding.recyclerView.apply {
            ...
            adapter = this@FavoriteSignaturesFragment.adapter
        }
    }

    private fun calculateSpanCount(): Int {
        val dm = resources.displayMetrics
        return ((dm.widthPixels / dm.density) / 180).toInt().coerceAtLeast(2)
    }

    private fun observeViewModel() {
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.favoriteSignatures.collect { sigs ->
