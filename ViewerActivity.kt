...
class ViewerActivity : AppCompatActivity() {
    ...
    private fun buildUI() {
        ...
        // Inline search bar (hidden by default) searchBar = buildSearchBar()
        ...
        // Page seek bar (merges Top/Prev/Next into one slider) pageSeekBar = SeekBar(this).apply { layoutParams = LinearLayout.LayoutParams(-1, dp(32))
        ...
        // Size slider sizeBar = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL
    ...
    private fun buildSearchBar(): LinearLayout {
        ...
        val btnSearch = Button(this).apply {
            text = "Search"; textSize = 10f
            setTextColor(Color.WHITE); setBackgroundColor(Color.parseColor("#1557B0"))
            setPadding(dp(8), 0, dp(8), 0)
            setOnClickListener {
                val query = searchBar.findViewWithTag<EditText>("search_et")?.text?.toString()?.trim() ?: ""
                if (query.isNotEmpty()) runSearch(query) else toast("Type something to search")
            }
        }
    ...
    private fun buildAnnotToolbarGrid(): AlertDialog {
        ...
        val tools = listOf(
            ...
            T(AnnotationCanvasView.TOOL_HIGHLIGHT, "Hi-Lite",   "#FFCC00", android.R.drawable.ic_menu_view),
            ...
            T("undo",                              "Undo",      "#888888", android.R.drawable.ic_menu_revert),
            T("redo",                              "Redo",      "#888888", android.R.drawable.ic_menu_rotate),
            T("save",                              "Save PDF",  "#2E7D32", android.R.drawable.ic_menu_save),
            T("extract",                           "Copy Text", "#1565C0", android.R.drawable.ic_menu_send),
        )

        // 4-column scrollable grid so all tools are visible on all screen sizes
        ...
        // ScrollView so all tools are reachable on small screens
        ...
        private val scaleGD = android.view.ScaleGestureDetector(ctx,
            object : android.view.ScaleGestureDetector.SimpleOnScaleGestureListener() {
                override fun onScale(d: android.view.ScaleGestureDetector): Boolean {
                    scaleFactor = (scaleFactor * d.scaleFactor).coerceIn(0.8f, 4f)
        ...
        override fun onTouchEvent(ev: android.view.MotionEvent): Boolean {
            ...
            if (ev.action == android.view.MotionEvent.ACTION_UP) {
                ...
                if (now - lastTapTime < 300L) {
                    if (scaleFactor > 1.1f) { scaleFactor=1f; scaleX=1f; scaleY=1f }
                    else { scaleFactor=2f; scaleX=2f; scaleY=2f }
                }
                lastTapTime = now
            }
            return super.onTouchEvent(ev) || scaleGD.isInProgress
        }
    }
}
