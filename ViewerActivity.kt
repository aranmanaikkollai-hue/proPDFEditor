...
class ViewerActivity : AppCompatActivity() {
    ...
    private fun buildAnnotBar(): LinearLayout {
        ...
        val tools = listOf(
            T("?? Draw",    AnnotationCanvasView.TOOL_FREEHAND,  "#1A73E8"),
        ...
        lifecycleScope.launch {
            try {
                ...
                for ((idx, cvs) in canvases) {
                    // Scale factor: how many view-pixels per PDF-point
                    // The bitmap fills screen width; PDF page width varies per document
                    // We use 1f here -- PdfOperationsManager divides coords by this scale.
                    // Actual scaling is handled by the renderer (bitmap == screen width).
                    val scale = 1f

                    val strokes = cvs.getStrokes()
                    if (strokes.isNotEmpty()) pageData[idx] = Pair(strokes, scale)

                    val textAnnots = cvs.getTextAnnots()
                    if (textAnnots.isNotEmpty()) pageTextData[idx] = Pair(textAnnots, scale)
                }

                if (pageData.isEmpty() && pageTextData.isEmpty()) {
                    progressBar.visibility = View.GONE; tvInfo.text = file.name
                    toast("Nothing to save -- add annotations first")
                    return@launch
                }
                ...
                pdfOps.saveAnnotationsToPdf(file, tmp, pageData, pageTextData).fold(
                    onSuccess = { savedTmp ->
                        ...
                        AlertDialog.Builder(this@ViewerActivity)
                            .setTitle("[OK] Saved to Downloads!")
