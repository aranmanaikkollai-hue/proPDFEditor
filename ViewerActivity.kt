...
class ViewerActivity : AppCompatActivity() {
    ...
    private suspend fun tryAndroidRenderer(file: File, password: String?): Boolean = try {
        withContext(Dispatchers.IO) {
            ...
            for (i in 0 until totalPages) {
                // Yield every 5 pages to let GC run and keep UI responsive
                if (i > 0 && i % 5 == 0) {
    ...
    private fun goToPage(page: Int) {
        ...
        // Use post() to ensure layout has completed before measuring heights
        scrollView.post {
            var cum = 0
            for (i in 0 until t) {
                cum += pageContainer.getChildAt(i)?.height ?: 0
            }
            // Use scrollTo for accuracy (smoothScrollTo can overshoot on tall pages)
