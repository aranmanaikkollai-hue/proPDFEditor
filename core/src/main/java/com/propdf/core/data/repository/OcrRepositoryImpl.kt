...
class OcrRepositoryImpl @Inject constructor(
    ...
    override suspend fun recognizeText(bitmap: Bitmap, language: String): AppResult<String> =
    ...
    override suspend fun findPagesContaining(bitmaps: List<Bitmap>, query: String): AppResult<List<Int>> =
        withContext(dispatchers.default) {
            ...
            for ((i, bmp) in bitmaps.withIndex()) {
                ...
                when (result) {
                    is AppResult.Success -> {
                        if (result.data.lowercase().contains(lowerQuery)) matching.add(i)
                    }
                    else -> { /* skip errors */ }
                }
            }
    ...
    override suspend fun recognizeFromUri(context: Context, uri: Uri): AppResult<String> =
        withContext(dispatchers.io) {
            if (uri.toString().isBlank()) {
                return@withContext AppResult.Error(AppException.FileNotFound("Empty URI"))
            }
            val bmp: Bitmap? = try {
            ...
            if (bmp == null || bmp.isRecycled) {
                return@withContext AppResult.Error(
                    AppException.IOError("Could not decode image from URI")
                )
            }
            recognizeText(bmp)
        }

    override suspend fun getSupportedLanguages(): AppResult<List<String>> =
        AppResult.Success(listOf("eng"))
