...
import com.propdf.core.domain.model.PointF
...
class OcrRepositoryImpl @Inject constructor(
    ...
    // NOTE: com.google.mlkit.vision.text.*.{Devanagari,Japanese,Korean,Chinese}TextRecognizerOptions
    // implement TextRecognizerOptionsInterface, NOT com.google.mlkit.common.model.RemoteModel.
    // RemoteModelManager.getDownloadedModels()/download()/deleteDownloadedModel() are generic
    // over <T : RemoteModel>, so passing these Options objects/classes was a type mismatch
    // ("inferred type is TextRecognizerOptionsInterface but RemoteModel was expected") and is
    // not the correct API for this offline text-recognition library in the first place: these
    // script models download automatically, on demand, the first time recognizer.process() is
    // called for that script - there's no public RemoteModel-based handle to query/manage them
    // explicitly. So these methods are simplified to reflect that; they no longer touch
    // RemoteModelManager for script recognizers.

    override suspend fun isModelDownloaded(language: OcrLanguage): Boolean = withContext(dispatchers.io) {
        // Latin (bundled) is always ready; other scripts download transparently on first use.
        true
    }

    override fun downloadModel(language: OcrLanguage): Flow<AppResult<Int>> = flow {
        emit(AppResult.Loading)
        // No explicit RemoteModel handle exists for these script recognizers; the model
        // downloads automatically the first time recognizeImage()/recognizeImageUri() runs
        // for that script, so there is nothing to pre-fetch here.
    ...
    override suspend fun deleteModel(language: OcrLanguage): AppResult<Unit> = withContext(dispatchers.io) {
        // No RemoteModel handle to delete for script recognizers (see note above).
        AppResult.Success(Unit)
    }

    override suspend fun getDownloadedModels(): AppResult<List<OcrLanguage>> = withContext(dispatchers.io) {
        // All supported scripts are usable immediately (they download on first use),
        // so report every language as available.
        AppResult.Success(OcrLanguage.values().toList())
    }
    ...
    override suspend fun exportToDocx(
        ...
        runCatching {
            context.contentResolver.openOutputStream(outputUri)?.use { outputStream ->
                writeMinimalDocx(results, outputStream)
            } ?: throw IOException("Cannot open output stream")
    ...
    // Builds a minimal, valid .docx (an OOXML WordprocessingML package) by hand with
    // java.util.zip instead of Apache POI. poi-ooxml pulls in classes (poi's own
    // CleanerUtil, and transitively log4j-api's ServiceLoaderUtil) that use
    // MethodHandle.invoke/invokeExact - bytecode D8 refuses to dex below minSdk 26
    // ("MethodHandle.invoke and MethodHandle.invokeExact are only supported starting
    // with Android O"). Since this project's minSdk is 23, no desugaring or multidex
    // setting fixes that; the library simply cannot ship in this app. A .docx is just
    // a zip of a few small, well-known XML parts, so writing those directly avoids the
    // dependency entirely.
    private fun writeMinimalDocx(results: List<OcrPageResult>, outputStream: OutputStream) {
        val body = StringBuilder()
        results.forEachIndexed { index, result ->
            if (index > 0) {
                // Page break between each source image's results.
                body.append("<w:p><w:r><w:br w:type=\"page\"/></w:r></w:p>")
            }
            result.blocks.forEach { block ->
                body.append("<w:p><w:r><w:t xml:space=\"preserve\">")
                body.append(xmlEscape(block.text))
...
<Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="word/document.xml"/>
    ...
    // getModelForLanguage() was removed: it declared a RemoteModel? return type but built
    // *TextRecognizerOptions instances, which implement TextRecognizerOptionsInterface, not
    // RemoteModel - a genuine type mismatch, and an API that doesn't apply to these bundled
    // script recognizers anyway (see the note above isModelDownloaded()).
