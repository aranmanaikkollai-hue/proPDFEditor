...
class OcrRepositoryImpl @Inject constructor(
    ...
    override suspend fun performOcr(uri: Uri, language: String): Result<OcrRecord> = withContext(Dispatchers.IO) {
        runCatching {
            ...
            val recognizer = recognizerFor(listOf(OcrLanguage.fromCode(language)))
            val extractedText = if (isPdf) {
                extractTextFromPdf(uri, recognizer)
            } else {
    ...
    /** Renders each page of a PDF via PdfRenderer and OCRs it, since ML Kit's
     *  InputImage.fromFilePath cannot decode PDF binary data directly. */
    private suspend fun extractTextFromPdf(uri: Uri, recognizer: TextRecognizer): String {
        val pfd = context.contentResolver.openFileDescriptor(uri, "r")
            ?: throw AppException.IOError("Could not open $uri")
        pfd.use { descriptor ->
            android.graphics.pdf.PdfRenderer(descriptor).use { renderer ->
                val pageTexts = mutableListOf<String>()
                for (i in 0 until renderer.pageCount) {
                    renderer.openPage(i).use { page ->
                        val scale = 2 // render above screen density for better OCR accuracy
