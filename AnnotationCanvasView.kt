    ...
    private fun drawStroke(cvs: Canvas, s: Stroke) {
        when (s.tool) {
            TOOL_STAMP -> {
                ...
                cvs.drawText(s.paint.getTag()?.toString() ?: "STAMP",
            ...
            MotionEvent.ACTION_DOWN -> {
                ...
                when (tool) {
                    ...
                    TOOL_STAMP -> pendingStamp?.let { stamp ->
                        ...
                        paint.setTag(stamp)
                        val s = Stroke(p, paint, TOOL_STAMP)
...
// Extension to store stamp label in Paint tag
fun Paint.setTag(tag: Any) { this.density = this.density }  // workaround - stamp text stored as tool id
