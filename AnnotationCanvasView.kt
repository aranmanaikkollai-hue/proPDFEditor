...
class AnnotationCanvasView @JvmOverloads constructor(
    ...
    companion object {
        ...
        const val TOOL_MOVE_SHAPE = "move_shape"
    ...
    data class TextAnnot(var x: Float, var y: Float, val text: String, val color: Int, val sizePx: Float)
    ...
    private var draggedStroke : Stroke? = null
    private var dragOffX = 0f; private var dragOffY = 0f
    private val strokeBounds = android.graphics.RectF()
    ...
    override fun onDraw(canvas: Canvas) {
        val cacheCanvas = cacheCvs
        if (cacheDirty) {
            cacheCanvas?.drawColor(Color.TRANSPARENT, PorterDuff.Mode.CLEAR)
            strokes.forEach { s -> cacheCanvas?.let { drawStroke(it, s) } }
            textAnnots.forEach { t -> cacheCanvas?.let { drawTextAnnot(it, t, t == draggedAnnot) } }
            cacheDirty = false
        }
    ...
    private fun drawStroke(cvs: Canvas, s: Stroke) {
        when (s.tool) {
            TOOL_STAMP -> {
                ...
                cvs.drawText(s.stampText.ifEmpty { "STAMP" }, bounds.left, bounds.bottom, tp)
            }
    ...
    private fun drawShapePreview(canvas: Canvas, x1: Float, y1: Float, x2: Float, y2: Float) {
        ...
        when (tool) {
            ...
            TOOL_CIRCLE -> canvas.drawOval(minOf(x1,x2), minOf(y1,y2), maxOf(x1,x2), maxOf(y1,y2), p)
    ...
    override fun onTouchEvent(ev: MotionEvent): Boolean {
        ...
        when (ev.actionMasked) {
            MotionEvent.ACTION_DOWN -> {
                ...
                when (tool) {
                    ...
                    TOOL_STAMP -> pendingStamp?.let { stamp ->
                        ...
                        cacheCvs?.let { drawStroke(it, s) } ?: run { cacheDirty = true }
                        if (cacheCvs != null) cacheDirty = false
                    ...
                    TOOL_MOVE_SHAPE -> {
                        draggedStroke = findStrokeAt(x, y)
                        draggedStroke?.let { s ->
                            s.path.computeBounds(strokeBounds, true)
                            dragOffX = x - strokeBounds.left
                            dragOffY = y - strokeBounds.top
                        }
                    }
                }
            }
            MotionEvent.ACTION_MOVE -> when (tool) {
                ...
                TOOL_UNDERLINE, TOOL_STRIKEOUT -> { endX = x; endY = startY }
                TOOL_MOVE_TEXT -> draggedAnnot?.let { a ->
                    a.x = x - dragOffX; a.y = y - dragOffY; cacheDirty = true
                }
                TOOL_MOVE_SHAPE -> draggedStroke?.let { s ->
                    s.path.computeBounds(strokeBounds, true)
                    val dx = (x - dragOffX) - strokeBounds.left
                    val dy = (y - dragOffY) - strokeBounds.top
                    val matrix = android.graphics.Matrix()
                    matrix.setTranslate(dx, dy)
                    s.path.transform(matrix)
                    cacheDirty = true
                }
            }
            MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                ...
                when (tool) {
                    ...
                    TOOL_MOVE_SHAPE -> draggedStroke = null
                }
                inShape = false
            }
        }
