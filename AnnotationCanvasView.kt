...
class AnnotationCanvasView @JvmOverloads constructor(
    ...
    companion object {
        ...
        const val TOOL_MOVE_TEXT = "move_text"  // drag existing text annotation
    }

    data class Stroke(val path: Path, val paint: Paint, val tool: String)

    // FIX: TextAnnot is now mutable (var x/y) so it can be dragged to a new position
    data class TextAnnot(
        var x: Float, var y: Float,
        val text: String, val color: Int, val sizePx: Float,
        // FIX: no background box by default -- pure text with color only
        val showBackground: Boolean = false
    )

    private var tool               = TOOL_NONE
    private var toolColor          = Color.parseColor("#E53935")  // default red
    ...
    // FIX: text rendering without background box
    private fun drawTextAnnot(cvs: Canvas, t: TextAnnot, selected: Boolean = false) {
        val tp = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            // Use LINEAR_TEXT for correct Unicode / Tamil rendering
            isLinearText  = true
            isAntiAlias   = true
            color         = t.color
            textSize      = t.sizePx
        ...
        if (selected) {
            // Show a subtle selection highlight when being dragged
            val bounds = Rect()
            ...
            cvs.drawRoundRect(
                ...
                t.x + bounds.width() + 6f, t.y + 6f,
                4f, 4f, selPaint
            )
        }
        // FIX: draw text ONLY -- no background box, no border
            ...
            MotionEvent.ACTION_DOWN -> {
                ...
                when (tool) {
                    ...
                    TOOL_TEXT -> pendingText?.let { txt ->
                        val ta = TextAnnot(x, y - currentTextSize, txt, toolColor, currentTextSize)
                    ...
                    TOOL_MOVE_TEXT -> {
                        // Find the text annotation under the touch point
    ...
    private fun findAnnotAtPoint(x: Float, y: Float): TextAnnot? {
        // Find topmost text annotation containing the touch point
    ...
    private fun makeLivePaint(): Paint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        ...
        when (tool) {
            ...
            TOOL_ERASER    -> { color = Color.parseColor("#CCCCCC"); alpha = 80; strokeWidth = currentStrokeWidth }
