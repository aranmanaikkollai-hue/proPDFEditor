...
class AnnotationCanvasView @JvmOverloads constructor(
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        when (event.action) {
            MotionEvent.ACTION_DOWN -> { startX = x; startY = y; currentPath = Path().apply { moveTo(x, y) } }
            MotionEvent.ACTION_MOVE -> { currentPath?.quadTo(startX, startY, (x+startX)/2, (y+startY)/2); startX = x; startY = y }
            MotionEvent.ACTION_UP   -> { currentPath?.lineTo(x, y); currentPath?.let { paths.add(DrawPath(Path(it), makePaint())); undoStack.clear() }; currentPath = null }
        }
    ...
    private fun makePaint() = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        color = currentColor
        alpha = if (currentTool == AnnotationType.HIGHLIGHT) 100 else 220
        style = Paint.Style.STROKE
        this.strokeWidth = this@AnnotationCanvasView.strokeWidth
        strokeCap = Paint.Cap.ROUND; strokeJoin = Paint.Join.ROUND
    }

    fun setTool(tool: AnnotationType, color: Int) { currentTool = tool; currentColor = color }
