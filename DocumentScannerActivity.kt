...
class DocumentScannerActivity : AppCompatActivity() {
    ...
    private fun finishScan() {
        ...
        android.app.AlertDialog.Builder(this)
            .setTitle("? Save Scanned PDF")
            .setMessage("Enter a filename for your scan:")
            .setView(et)
    ...
    private fun doFinishScan(outName: String) {
        ...
        lifecycleScope.launch {
            try {
                ...
                pdfOps.imagesToPdf(imgFiles, tmpPdf)
                    .onSuccess { pdf ->
                        // Copy to Downloads so user can find it
                        val saved = withContext(Dispatchers.IO) {
                            com.propdf.editor.utils.FileHelper.saveToDownloads(
                                this@DocumentScannerActivity, pdf
                            )
                        }
                        // Clean up temp images
                        imgFiles.forEach { it.delete() }

                        android.app.AlertDialog.Builder(this@DocumentScannerActivity)
                            .setTitle("[OK] Scan Saved!")
                            .setMessage(
                                "? $outName.pdf\n\n" +
                            ...
                            .setPositiveButton("OK") { _, _ ->
                                setResult(RESULT_OK,
                                    Intent().putExtra("pdf_path", pdf.absolutePath))
                                finish()
                            }
                            .setNeutralButton("View PDF") { _, _ ->
                                ...
                                com.propdf.editor.ui.viewer.ViewerActivity.start(
                                    this@DocumentScannerActivity, uri
                                )
                                finish()
                            }
                            .show()
                    ...
                    .onFailure {
                        toast("[X] Failed to create PDF: ${it.message}")
                        btnDone.isEnabled = true
                    }

            } catch (e: Exception) {
                toast("[X] Error: ${e.message}")
                btnDone.isEnabled = true
            } finally {
