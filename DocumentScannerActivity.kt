// FILE: DocumentScannerActivity.kt (v4.1 -- build-error-fixed)
...
//   1. Added explicit import androidx.camera.core.ImageProxy  (was missing -> lines 52,68,89 errors)
//   2. Replaced proxy.toBitmap() with manual RGBA_8888 pixel copy
//      (toBitmap() is an extension that may not resolve without star-import)
//   3. EdgeOverlayView and ScanSaveActivity are now resolved via codemagic.yaml cp commands
//
// CRITICAL RULES OBEYED:
//   Rule #13: explicit import androidx.camera.core.Camera after wildcard
//   Rule #10: FrameLayout.LayoutParams(width, height) only -- no weight float
//   Rule #14: isVerticalScrollBarEnabled = false (not scrollbars property)
//   Rule #32: pure ASCII -- no smart quotes, no em-dashes
...
import androidx.camera.core.ImageProxy               // FIX: was missing -- caused lines 52,68,89
...
class DocumentScannerActivity : AppCompatActivity() {
    ...
    private fun buildUI() {
        ...
        // Tap-to-focus transparent layer (rule from bug #22 -- prevents capture btn block)
        ...
        focusTapLayer.setOnTouchListener { _, event ->
            ...
            camera?.cameraControl?.startFocusAndMetering(action) // rule #13: camera is Camera type
            false
        }
    }
    ...
    // replaces proxy.toBitmap() which requires an extension import that may not resolve
    ...
    private fun analyzeFrame(proxy: ImageProxy) {
        ...
        if (now - lastAnalysisMs < 400L) {       // 400ms throttle (~2.5 fps analysis)
            proxy.close(); return
        }
        lastAnalysisMs = now
        ...
        lifecycleScope.launch(Dispatchers.Default) {
            ...
            // Update overlay (EdgeOverlayView.updateDetection posts to UI thread internally)
            edgeOverlay.updateDetection(result.quad, proxy.width, proxy.height, result.isBlurry)
    ...
    private fun captureImage() {
        if (lastBlurry) {
            Toast.makeText(this, "Image blurry -- hold still", Toast.LENGTH_SHORT).show()
            return
        }
