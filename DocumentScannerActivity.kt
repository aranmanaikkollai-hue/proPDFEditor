...
class DocumentScannerActivity : AppCompatActivity() {
    ...
    private fun imageProxyToBitmap(proxy: ImageProxy): Bitmap {
        // Use JPEG bytes if available (most reliable, no divide-by-zero risk)
        ...
        // Fallback: manual ARGB copy using proxy.width to avoid divide-by-zero
        val w   = proxy.width.coerceAtLeast(1)
        val h   = proxy.height.coerceAtLeast(1)
        val bmp = Bitmap.createBitmap(w, h, Bitmap.Config.ARGB_8888)
        try { bmp.copyPixelsFromBuffer(plane.buffer.rewind()) } catch (_: Exception) {}
        return bmp
    }
