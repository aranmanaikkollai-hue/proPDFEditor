...
class DocumentScannerActivity : AppCompatActivity() {
    ...
    private fun buildUI() {
        ...
        // FIX: Tap-to-focus on the preview -- use previewView directly, NOT its children
        // The bottom overlay is SEPARATE in FrameLayout, so touches on top half hit previewView
        previewView.setOnTouchListener { _, ev ->
            if (ev.action == MotionEvent.ACTION_UP) {
                // Only focus if tap is not in the bottom controls area
                val bottomAreaView = root.getChildAt(root.childCount - 1) // bottomArea
                if (ev.y < (root.height - bottomAreaView.height)) {
                    tapToFocus(ev.x, ev.y)
                }
            }
            // Return false so other gestures still work
            false
        }
