...
class SelectionTool {
    ...
    /**
     * Replaces the selection's cached annotation objects with their updated versions
     * (same identity/order), without changing which annotations are selected.
     *
     * This must be called after every move/scale/rotate/style change applied to the
     * selection. Without it, moveSelection/scaleSelection/rotateSelection keep computing
     * deltas from the stale, pre-gesture annotation objects on every call — so during a
     * continuous drag, each frame's tiny incremental delta gets applied to the *original*
     * position instead of accumulating, and the annotation barely moves instead of
     * following the finger.
     */
    fun replaceSelection(updated: List<Annotation>) {
        _selectedAnnotations.clear()
        _selectedAnnotations.addAll(updated)
    }
    ...
    /** Per-axis scale -- see Annotation.scaleXY for why this exists (edge vs corner handles). */
    fun scaleSelectionXY(factorX: Float, factorY: Float, pivotX: Float, pivotY: Float): List<Annotation> {
        return _selectedAnnotations.map { it.scaleXY(factorX, factorY, pivotX, pivotY) }
    }
