...
/**
 * Command to modify an annotation (replace with updated version).
 * Preserves the original for undo.
 */
class ModifyAnnotationCommand(
    ...
    override fun getDescription(): String = "Modify ${oldAnnotation.type.name.lowercase().replace("_", " ")}"

    override fun getAffectedIds(): List<String> = listOf(oldAnnotation.id)
}
