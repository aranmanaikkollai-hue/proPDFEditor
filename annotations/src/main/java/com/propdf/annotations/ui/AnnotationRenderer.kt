...
class AnnotationRenderer {
    ...
    private fun DrawScope.renderShape(
        ...
        when (annotation.shapeType) {
            ShapeAnnotation.ShapeType.RECTANGLE -> {
                ...
                fillPaint?.let { drawIntoCanvas { it.nativeCanvas.drawRect(rect, it) } }
                drawIntoCanvas { it.nativeCanvas.drawRect(rect, paint) }
            }
            ShapeAnnotation.ShapeType.CIRCLE -> {
                val rect = annotation.rect.toScreen(pageScale, pageOffset)
                fillPaint?.let { drawIntoCanvas { it.nativeCanvas.drawOval(rect, it) } }
                drawIntoCanvas { it.nativeCanvas.drawOval(rect, paint) }
            }
            ...
            ShapeAnnotation.ShapeType.POLYGON -> {
                if (annotation.vertices.size >= 3) {
                    ...
                    fillPaint?.let { drawPath(path, Color(it.color), style = Fill, alpha = annotation.opacity) }
            ...
            ShapeAnnotation.ShapeType.CLOUD -> {
                if (annotation.vertices.size >= 3) {
                    ...
                    fillPaint?.let { drawPath(path, Color(it.color), style = Fill, alpha = annotation.opacity * 0.1f) }
                    drawPath(path, color, style = Stroke(strokeWidth), alpha = annotation.opacity)
                }
            }
        }
    ...
    private fun DrawScope.drawVariableWidthStroke(
        ...
        val path = pathCache.getOrPut(annotation.id) { Path() }
        path.reset()
    ...
    private fun DrawScope.renderText(
        ...
        lines.forEachIndexed { index, line ->
            ...
            drawIntoCanvas { it.nativeCanvas.drawText(line, x, y, textPaint) }
        }
    }
    ...
    private fun createCloudPath(
        ...
        for (i in 0 until vertices.size) {
            ...
            val midX = (current.x + next.x) / 2 * pageScale + pageOffset.x
            val midY = (current.y + next.y) / 2 * pageScale + pageOffset.y
