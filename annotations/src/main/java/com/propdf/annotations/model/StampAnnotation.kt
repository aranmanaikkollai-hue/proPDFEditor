...
data class StampAnnotation(
    ...
    // Date Stamp configuration -- previously the display text always used a hardcoded
    // "MMM dd, yyyy" pattern and always showed "now". See DateFormatOption for the
    // shared pattern catalog/formatter.
    val dateFormatPattern: String = DateFormatOption.DEFAULT_PATTERN,
    val customDateMillis: Long? = null, // null = use the current date at render/display time
    ...
    fun getDisplayText(): String {
        return when (stampType) {
            ...
            StampType.DATE_STAMP -> {
                val date = customDateMillis?.let { java.util.Date(it) } ?: java.util.Date()
                DateFormatOption.format(dateFormatPattern, date)
            }
        }
    }
