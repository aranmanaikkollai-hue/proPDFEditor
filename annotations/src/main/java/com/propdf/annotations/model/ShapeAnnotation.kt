...
data class ShapeAnnotation(
    ...
    override fun scaleXY(factorX: Float, factorY: Float, pivotX: Float, pivotY: Float): Annotation = copy(
        rect = RectF(
            pivotX + (rect.left - pivotX) * factorX,
            pivotY + (rect.top - pivotY) * factorY,
            pivotX + (rect.right - pivotX) * factorX,
            pivotY + (rect.bottom - pivotY) * factorY
        ),
        vertices = vertices.map {
            PointF(
                pivotX + (it.x - pivotX) * factorX,
                pivotY + (it.y - pivotY) * factorY,
                it.pressure
            )
        },
        strokeWidth = strokeWidth * ((factorX + factorY) / 2f),
        modifiedAt = currentTime()
    )
