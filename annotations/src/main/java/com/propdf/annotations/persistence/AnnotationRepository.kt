...
class AnnotationRepository(
     ...
     *
     * This is a full replace of the document's persisted annotation set, not an upsert:
     * existing rows for this documentId are deleted first, then the current in-memory
     * annotations are inserted. `annotations` is always the complete authoritative state
     * (AnnotationViewModel.saveAnnotations() always passes layerManager.getAllAnnotations()),
     * so a plain insert-with-REPLACE here would leave orphaned rows in Room for any
     * annotation the caller removed (single delete, multi-delete, or a whole deleted layer) --
     * those orphaned rows would then silently resurrect the "deleted" annotation on the next
     * document load, and get baked into output by flattenAnnotations/burnAnnotationsIntoPdf,
     * which both read straight from this table by documentId rather than from the live layer.
     */
