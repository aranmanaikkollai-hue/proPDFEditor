...
/**
 * Bottom sheet dialog for PDF tools (merge, split, compress, etc.).
 * Provides UI for all operations with progress indicators.
 */
class PdfToolsBottomSheet : BottomSheetDialogFragment() {
    ...
    private var selectedPages: List<Int> = emptyList()
    ...
    private fun setupTabs() {
        val tabs = listOf("Compress", "Split", "Extract", "Rotate", "Delete", "Merge")
        // Simple tab switching via chip group or similar
    }
    ...
    private fun setupPageSelection() {
        // Page range input for extract/delete/rotate
        binding.pageRangeInput.setOnClickListener {
            showPageRangeDialog()
        }
    }
    ...
    private fun setupActions() {
        ...
        binding.btnExtract.setOnClickListener {
            currentSourceUri?.let { uri ->
                ...
                if (pages.isNotEmpty()) {
                    viewModel.extractPages(uri, pages)
                }
            }
        }
        ...
        binding.btnDelete.setOnClickListener {
            currentSourceUri?.let { uri ->
                val pages = parsePageRange(binding.pageRangeInput.text.toString(), currentPageCount)
                if (pages.isNotEmpty()) {
                    viewModel.deletePages(uri, pages)
                }
            }
        }
    ...
    private fun showPageRangeDialog() {
        // Show dialog to select page range
    }
