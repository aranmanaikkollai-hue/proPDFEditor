 ...
 */
class BitmapPool internal constructor() {
    ...
    data class BitmapConfigKey(
        ...
        val byteSize: Long = width.toLong() * height * when (config) {
    ...
    /**
     * Non-suspend release for use from non-coroutine contexts (cache eviction callbacks).
     * Note: not mutex-protected to avoid blocking; safe for single-threaded eviction callers.
     */
    @WorkerThread
