...
class AnnotationPersistenceManager(private val annotationDao: AnnotationDao) {
    ...
    suspend fun loadAnnotations(pdfUri: String): List<Annotation> = withContext(Dispatchers.IO) {
        try {
            ioMutex.withLock {
                annotationDao.getByPdfUri(pdfUri).mapNotNull { deserializeAnnotation(JSONObject(it.payloadJson)) }
            }
        } catch (e: Exception) {
            e.printStackTrace()
            emptyList()
