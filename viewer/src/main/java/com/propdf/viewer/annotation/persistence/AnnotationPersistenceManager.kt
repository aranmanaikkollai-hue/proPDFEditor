...
class AnnotationPersistenceManager(
    ...
    suspend fun saveAnnotations(pdfUri: String, annotations: List<Annotation>) = withContext(Dispatchers.IO) {
        try {
            ioMutex.withLock {
                val rows = annotations.map { annotation ->
                    AnnotationEntity(
                        id = annotation.id,
                        pdfUri = pdfUri,
                        pageIndex = annotation.pageIndex,
                        payloadJson = serializeAnnotation(annotation).toString(),
                        updatedAt = System.currentTimeMillis()
                    )
                }
                annotationDao.replaceDocumentAnnotations(pdfUri, rows)
                true
            }
        } catch (e: Exception) {
            e.printStackTrace()
    ...
    suspend fun loadAnnotations(pdfUri: String): List<Annotation> = withContext(Dispatchers.IO) {
        try {
            ioMutex.withLock {
                val rows = annotationDao.getByPdfUri(pdfUri)
                if (rows.isNotEmpty()) {
                    rows.mapNotNull { deserializeAnnotation(JSONObject(it.payloadJson)) }
                } else {
                    loadLegacyJsonAnnotations(pdfUri).also { legacyAnnotations ->
                        if (legacyAnnotations.isNotEmpty()) {
                            val migratedRows = legacyAnnotations.map { annotation ->
                                AnnotationEntity(
                                    id = annotation.id,
                                    pdfUri = pdfUri,
                                    pageIndex = annotation.pageIndex,
                            ...
                            annotationDao.replaceDocumentAnnotations(pdfUri, migratedRows)
                        }
                    }
                }
            }
    ...
    suspend fun deleteAnnotations(pdfUri: String) = withContext(Dispatchers.IO) {
        ioMutex.withLock { annotationDao.deleteByPdfUri(pdfUri) }
    }

    private fun loadLegacyJsonAnnotations(pdfUri: String): List<Annotation> {
        val file = File(File(context.filesDir, "annotations"), "${pdfUri.hashCode()}.json")
        ...
        return try {
            ...
            buildList {
                for (i in 0 until jsonArray.length()) {
                    deserializeAnnotation(jsonArray.getJSONObject(i))?.let { add(it) }
                }
            }
        } catch (e: Exception) {
            emptyList()
    ...
    private fun serializeAnnotation(annotation: Annotation): JSONObject {
        ...
        when (annotation) {
            is TextMarkupAnnotation -> {
                obj.put("rectBounds", serializeRect(annotation.rectBounds))
            ...
            when (type) {
                AnnotationType.HIGHLIGHT, AnnotationType.UNDERLINE, AnnotationType.STRIKEOUT -> {
                    TextMarkupAnnotation(
                        ...
                        createdAt = createdAt,
                        zIndex = zIndex,
                        rectBounds = deserializeRect(obj.getJSONObject("rectBounds")),
