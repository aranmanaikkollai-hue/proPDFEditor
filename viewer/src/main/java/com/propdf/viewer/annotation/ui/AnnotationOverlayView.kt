...
/**
* Hardware-accelerated annotation overlay that sits on top of PDF page views.
* Uses SurfaceView for async rendering to eliminate touch lag.
* Handles all annotation touch events and renders annotations at 60fps.
*/
...
private lateinit var touchEngine: AnnotationTouchEngine
...
// Scale from PDF coordinates to view coordinates
var scaleX: Float = 1f
var scaleY: Float = 1f
var offsetX: Float = 0f
var offsetY: Float = 0f

// Callback for viewer gesture passthrough
...
// Calculate actual page bounds on this view
val pageLeft = offsetX
val pageTop = offsetY
val pageRight = pageLeft + pdfPageWidth * scaleX
val pageBottom = pageTop + pdfPageHeight * scaleY
...
// Check if touch is within page bounds
val x = event.x
val y = event.y
val pageLeft = offsetX
val pageTop = offsetY
...
if (!withinPage) {
// Pass to viewer for scrolling
return onViewerTouchEvent?.invoke(event) ?: false
}

// Convert to view-local coordinates for touch engine
