...
/**
* Base annotation model. All annotation types extend this.
* Uses normalized coordinates (0.0-1.0) for PDF coordinate independence.
*/
...
data class TextMarkupAnnotation(
    ...
    override fun scale(scaleFactor: Float, pivotX: Float, pivotY: Float): Annotation = copy(
        bounds = scaleRect(bounds, scaleFactor, pivotX, pivotY),
    ...
    override fun scale(scaleFactor: Float, pivotX: Float, pivotY: Float): Annotation = copy(
        strokes = strokes.map { stroke: Stroke ->
            stroke.copy(points = stroke.points.map { pt: PointF ->
                PointF(
                    pivotX + (pt.x - pivotX) * scaleFactor,
                    pivotY + (pt.y - pivotY) * scaleFactor
                )
            })
        }
    )
