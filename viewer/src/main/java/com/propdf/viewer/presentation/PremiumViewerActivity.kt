...
import com.propdf.viewer.annotation.ui.AnnotationToolbarView
 ...
 * - Annotation toolbar with unified gesture coordination
 */
@AndroidEntryPoint
class PremiumViewerActivity : AppCompatActivity() {
    ...
    @javax.inject.Inject
    lateinit var persistenceManager: AnnotationPersistenceManager

    @javax.inject.Inject
    lateinit var annotationFlattener: AnnotationFlattener

    private lateinit var pageAdapter: PremiumPageAdapter
    private lateinit var thumbnailAdapter: ThumbnailAdapter
    private lateinit var tabAdapter: TabAdapter
    private lateinit var annotationToolbar: AnnotationToolbarView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (filePath != null) {
            ...
            viewModel.loadDocument(currentPdfFile!!)
        }
    }
    ...
    private fun setupSystemBars() {
        ...
        ViewCompat.setOnApplyWindowInsetsListener(binding.root) { _, insets ->
            ...
            binding.bottomControls.updatePadding(bottom = systemBars.bottom)
            insets
        }
    }
    ...
    private fun setupAnnotationToolbar() {
        annotationToolbar = AnnotationToolbarView(this).apply {
            layoutParams = ViewGroup.LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.WRAP_CONTENT
            )
        }
        // Add above bottom controls; adjust margin so it does not overlap
        (binding.root as? ViewGroup)?.addView(annotationToolbar)
        val marginLarge = resources.getIdentifier("margin_large", "dimen", packageName)
        val translationY = if (marginLarge != 0) {
            -resources.getDimensionPixelSize(marginLarge).toFloat()
        } else {
            -16f
        }
        annotationToolbar.translationY = translationY

        annotationToolbar.setOnToolSelectedListener { tool ->
            pageAdapter.setTool(tool)
        }
        annotationToolbar.setOnColorSelectedListener { color ->
            pageAdapter.setColor(color)
        }
        annotationToolbar.setOnStrokeWidthListener { width ->
            pageAdapter.setStrokeWidth(width)
        }
        annotationToolbar.setOnUndoListener {
            pageAdapter.undo()
            updateUndoRedoState()
        }
        annotationToolbar.setOnRedoListener {
            pageAdapter.redo()
            updateUndoRedoState()
        }
        annotationToolbar.setOnClearListener {
            pageAdapter.clearAll()
            updateUndoRedoState()
        }
        annotationToolbar.setOnExportListener {
            exportAnnotations()
        }
    }

    private fun updateUndoRedoState() {
        annotationToolbar.setUndoEnabled(annotationManager.canUndo())
        annotationToolbar.setRedoEnabled(annotationManager.canRedo())
    }
    ...
    private fun exportAnnotations() {
        ...
        lifecycleScope.launch {
            try {
                val outFile = File(cacheDir, "annotated_${file.name}")
                annotationFlattener.flattenAnnotations(
                    Uri.fromFile(file),
                    annotationManager,
                    outFile
                )
                /* TODO: Show share/save dialog for outFile */
            } catch (e: Exception) {
                e.printStackTrace()
            }
        }
    ...
    private fun updateUi(state: PremiumViewerViewModel.PremiumViewerUiState) {
        ...
        if (thumbnailAdapter.currentList.size != state.totalPages) {
            thumbnailAdapter.submitList((0 until state.totalPages).map { ThumbnailItem(pageIndex = it) })
        }
        thumbnailAdapter.setSelectedPosition(state.currentPage)
        ...
        // Tabs
        tabAdapter.submitList(state.tabs)
        tabAdapter.setSelectedPosition(state.activeTabIndex)
        ...
        // Annotations
        if (state.documentName.isNotBlank() && currentPdfFile != null) {
            pageAdapter.setPdfUri(Uri.fromFile(currentPdfFile!!))
        }
        updateUndoRedoState()
    }
    ...
    private fun handleEvent(event: PremiumViewerViewModel.ViewerEvent) {
        when (event) {
            ...
            is PremiumViewerViewModel.ViewerEvent.PageRendered -> {
                pageAdapter.setBitmap(event.pageIndex, event.bitmap)
            }
            is PremiumViewerViewModel.ViewerEvent.ThumbnailRendered -> {
                updateThumbnail(event.pageIndex, event.bitmap)
            }
        }
    }
    ...
    private fun toggleUiVisibility() {
        ...
        // Toggle annotation toolbar visibility with bottom controls
        annotationToolbar.animate()
            .alpha(if (isUiVisible) 1f else 0f)
            .translationY(if (isUiVisible) 0f else annotationToolbar.height.toFloat())
    ...
    private fun updateThumbnail(pageIndex: Int, bitmap: android.graphics.Bitmap) {
        val totalPages = viewModel.uiState.value.totalPages
        if (pageIndex !in 0 until totalPages || bitmap.isRecycled) return

        val items = if (thumbnailAdapter.currentList.size == totalPages) {
            thumbnailAdapter.currentList.toMutableList()
        } else {
            MutableList(totalPages) { index -> ThumbnailItem(pageIndex = index) }
        }
        items[pageIndex] = ThumbnailItem(pageIndex = pageIndex, bitmap = bitmap)
