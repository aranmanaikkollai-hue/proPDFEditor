...
class PdfViewerRepositoryImpl @Inject constructor(
    ...
    suspend fun copyUriToCache(uri: Uri): AppResult<File> = withContext(dispatchers.io) {
        try {
            if (uri.scheme == "file") {
            ...
            if (dest.exists() && dest.length() > 0L) AppResult.Success(dest)
            else AppResult.Error(AppException.FileNotFound())
        } catch (e: Exception) {
    ...
    suspend fun getPageCount(file: File): AppResult<Int> = withContext(dispatchers.io) {
        try {
            ParcelFileDescriptor.open(file, ParcelFileDescriptor.MODE_READ_ONLY).use { pfd ->
                PdfRenderer(pfd).use { AppResult.Success(it.pageCount) }
            }
        } catch (e: Exception) {
    ...
    suspend fun renderPage(file: File, pageIndex: Int, screenWidth: Int): AppResult<Bitmap> =
        withContext(dispatchers.io) {
            try {
                pageBitmapCache.get(pageIndex)?.takeIf { !it.isRecycled }
    ...
    suspend fun preloadPages(file: File, anchorPage: Int, screenWidth: Int): AppResult<Unit> =
        withContext(dispatchers.io) {
            try {
                val max = (getPageCount(file) as? AppResult.Success)?.data?.minus(1) ?: 0
                for (i in (anchorPage - 2).coerceAtLeast(0)..(anchorPage + 2).coerceAtMost(max)) {
                    if (pageBitmapCache.get(i) == null) renderPage(file, i, screenWidth)
                }
