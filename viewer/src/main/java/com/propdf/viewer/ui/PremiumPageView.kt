 ...
 * - Unified annotation rendering (same canvas, same transform)
 */
class PremiumPageView @JvmOverloads constructor(
    ...
    var pageIndex: Int = 0
    val pdfCoordinateSpace = PdfCoordinateSpace()
    // ===================================
    ...
    private fun updateMatrix() {
        ...
        if (bitmap != null && !bitmap.isRecycled) {
            pdfCoordinateSpace.update(transformMatrix, bitmap.width.toFloat(), bitmap.height.toFloat())
        }
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        // ===== UNIFIED ANNOTATION RENDERING =====
        // Annotations draw in the same transformed canvas space as the PDF bitmap,
        // guaranteeing pixel-perfect alignment regardless of zoom or pan.
        val annMgr = annotationManager
        val annRenderer = annotationRenderer
        if (annMgr != null && annRenderer != null) {
            annRenderer.render(
                canvas = canvas,
                annotationManager = annMgr,
                pageIndex = pageIndex,
