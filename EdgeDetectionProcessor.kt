// FILE: EdgeDetectionProcessor.kt
...
//   2. Perspective warp (4-point homography using android.graphics.Matrix)
...
class EdgeDetectionProcessor @Inject constructor() {
    ...
    suspend fun blurScore(src: Bitmap): Double = withContext(Dispatchers.Default) {
    ...
    fun isBlurry(score: Double, threshold: Double = 80.0) = score < threshold
    ...
    suspend fun detectDocumentCorners(src: Bitmap): EdgeResult = withContext(Dispatchers.Default) {
    ...
    suspend fun perspectiveWarp(src: Bitmap, quad: Quad): Bitmap = withContext(Dispatchers.Default) {
        val tl = quad.topLeft
    ...
    private fun cannyEdges(
        ...
        for (y in 1 until h - 1) {
            for (x in 1 until w - 1) {
                ...
                val gyv = -src[(y-1)*w+(x-1)] - 2*src[(y-1)*w+x] - src[(y-1)*w+(x+1)] +
    ...
    // Strategy: sample edge pixels, build convex hull, approximate to 4 corners.
