...
class OcrManager @Inject constructor() {
    ...
    suspend fun recognizeFromUri(context: android.content.Context, uri: android.net.Uri): Result<String> {
        if (uri.toString().isBlank()) return Result.failure(IllegalArgumentException("Empty URI"))
        val bmp: android.graphics.Bitmap? = try {
            context.contentResolver.openInputStream(uri)?.use {
                android.graphics.BitmapFactory.decodeStream(it)
            }
        } catch (e: Exception) { return Result.failure(e) }
        if (bmp == null || bmp.isRecycled) return Result.failure(IllegalStateException("Could not decode image"))
        return recognizeText(bmp)
    }
