...
class ToolsActivity : AppCompatActivity() {
    ...
    private fun buildUI() {
        ...
        tvStatus = TextView(this).apply {
            textSize = 13f; setTextColor(0xFF555555.toInt())
            text = statusText(); setPadding(0, dp(4), 0, dp(4))
        }
        ...
        root.addView(card("Page Tools", listOf(
            btn("Rotate pages (preview)",       0xFF37474F.toInt()) { doRotate() },
    ...
    private fun doCompress() {
        val f = need() ?: return
        AlertDialog.Builder(this).setTitle("Compress Level")
            .setItems(arrayOf(
                "High quality (smaller reduction)",
                "Medium quality (good reduction)",
                "Low quality (max reduction)"
            )) { _, which ->
                    ...
                    run("Compressing (level $level)...") {
                        pdfOps.compressPdf(f, FileHelper.tempFile(this@ToolsActivity, name), level)
                            .onSuccess {
                                ...
                                val msg = if (pct > 0) "Compressed $pct% smaller"
                                else "File rewritten (already well-compressed)"
                                done(it, name, msg)
                            }
    ...
    private fun doWatermark() {
        ...
        val etOpac = EditText(this).apply {
            setText("30"); hint = "Opacity % (1-100)"
    ...
    private fun makeWatermarkPreview(text: String, opacity: Float): Bitmap {
        ...
        val paint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            color = Color.GRAY; alpha = (opacity * 255).toInt()
            textSize = 60f; typeface = Typeface.DEFAULT_BOLD
        }
        cvs.save(); cvs.rotate(-35f, 300f, 150f)
        cvs.drawText(text.take(18), 40f, 165f, paint)
        cvs.restore(); return bmp
    }
    ...
    private fun doRotate() {
        ...
        rg.setOnCheckedChangeListener { grp, _ ->
            val deg = when (grp.checkedRadioButtonId) {
                rg.getChildAt(0).id -> 90
                rg.getChildAt(1).id -> 180
                else -> 270
            }
            ...
            .setPositiveButton("Next") { _, _ ->
                askFilename("rotated_${selectedDeg}deg") { name ->
                    run("Rotating ${selectedDeg}...") {
                        pdfOps.rotatePages(f, FileHelper.tempFile(this@ToolsActivity, name),
                            (1..n).associateWith { selectedDeg })
                            .onSuccess { done(it, name) }
    ...
    private fun makeRotatePreview(deg: Int): Bitmap {
        ...
        val cvs = Canvas(bmp); cvs.drawColor(Color.WHITE)
        val paint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            color = Color.parseColor("#1A73E8"); textSize = 40f
        }
        ...
        val arrowP = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            color = Color.parseColor("#E53935"); textSize = 40f
        }
        cvs.drawText("--> $deg", 220f, 115f, arrowP)
        return bmp
    }
    ...
    private fun doPageNumbers() {
        ...
        val etFmt = EditText(this).apply { setText("Page %d of %d"); hint = "Format (%d = page number)" }
    ...
    private fun doHeaderFooter() {
        ...
        val tvH = TextView(this).apply { text = "[ header ]"; textSize = 11f; setTextColor(Color.parseColor("#1A73E8")) }
        val tvB = TextView(this).apply { text = "\n   Page content...\n"; textSize = 11f }
        val tvF = TextView(this).apply { text = "[ footer ]"; textSize = 11f; setTextColor(Color.parseColor("#E53935")) }
        previewCard.addView(tvH); previewCard.addView(tvB); previewCard.addView(tvF)
        val watcher = object : android.text.TextWatcher {
            override fun afterTextChanged(s: android.text.Editable?) {
                tvH.text = etH.text.toString().ifBlank { "[ no header ]" }
                tvF.text = etF.text.toString().ifBlank { "[ no footer ]" }
            }
            override fun beforeTextChanged(a: CharSequence?, b: Int, c: Int, d: Int) {}
            override fun onTextChanged(a: CharSequence?, b: Int, c: Int, d: Int) {}
        }
        ...
        run("Exporting pages as JPG...") {
            try {
                ...
                for (i in 0 until r.pageCount) {
                    ...
                    pg.close()
                    // FIX: correct filename WITHOUT .pdf extension
                    ...
                    bmp.recycle()
                    // FIX: save with image/jpeg MIME type via updated FileHelper
                ...
                withContext(Dispatchers.Main) {
                    AlertDialog.Builder(this@ToolsActivity)
                        .setTitle("Done!")
                        .setMessage(
                            "$count images saved to Downloads.\n\n" +
                            "Files named:\n" +
                            "${baseName}_page1.jpg\n${baseName}_page2.jpg\n..."
                        )
    ...
    private fun askFilename(default: String, cb: (String) -> Unit) {
        val et = EditText(this).apply {
            setText(default); selectAll(); setPadding(dp(20), dp(8), dp(20), dp(8))
        }
        AlertDialog.Builder(this).setTitle("Save as...")
            .setMessage("Enter filename (without .pdf):")
            .setView(et)
            .setPositiveButton("Save") { _, _ -> cb(et.text.toString().trim().ifBlank { default }) }
            .setNegativeButton("Cancel", null).show()
    }
        ...
        lifecycleScope.launch {
            ...
            val saved = withContext(Dispatchers.IO) {
                try { FileHelper.saveToDownloads(this@ToolsActivity, namedFile) }
                    ...
                    FileHelper.SaveResult("app storage", Uri.fromFile(namedFile), namedFile)
                }
            }
    ...
    private fun statusText() =
        if (files.isEmpty()) "No file selected" else "${files.size} file(s) ready"
