...
class PdfOperationsManager @Inject constructor(
    ...
    // ---- COMPRESS -------------------------------------------------------
    // FIX: use full compression mode + object streams + smart mode for real size reduction
    suspend fun compressPdf(inputFile: File, outputFile: File, quality: Int = 9): Result<File> =
        withContext(Dispatchers.IO) { runCatching {
            val props = WriterProperties().apply {
                ...
                setFullCompressionMode(true)  // enables object streams (critical for compression)
                useSmartMode()               // avoids duplicating identical objects
            }
            // Read with StampingProperties to preserve structure
            val reader = PdfReader(inputFile.absolutePath)
            val writer = PdfWriter(outputFile.absolutePath, props)
            val doc    = PdfDocument(reader, writer,
                StampingProperties().useAppendMode().apply {  } )
            // Actually just copy and rewrite -- append mode doesn't compress
            // Use standard rewrite instead
            doc.close()

            // If append-mode output is larger, fall back to clean rewrite
            if (outputFile.length() >= inputFile.length()) {
                outputFile.delete()
                val props2 = WriterProperties().apply {
                ...
                val doc2 = PdfDocument(
                    ...
                    PdfWriter(outputFile.absolutePath, props2)
                )
                try { } finally { doc2.close() }
            }
            outputFile
        }}
    ...
    // ---- IMAGES TO PDF --------------------------------------------------
    // FIX: create page sized to image, draw image directly via PdfCanvas
        ...
        withContext(Dispatchers.IO) { runCatching {
            ...
            try {
                imageFiles.filter { it.exists() && it.length() > 0 }.forEach { f ->
                    ...
                    // FIX: size the page to match the image dimensions
                    val imgW    = imgData.width
                    val imgH    = imgData.height
                    val pageSize = PageSize(imgW, imgH)
                    val page     = pdfDoc.addNewPage(pageSize)
                    val canvas   = PdfCanvas(page, pdfDoc)
                    try {
                        // FIX: draw image at 0,0 filling the entire page
                        canvas.addImageAt(imgData, 0f, 0f, false)
                    } finally { canvas.release() }
                }
