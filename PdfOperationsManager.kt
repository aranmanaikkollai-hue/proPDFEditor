...
class PdfOperationsManager @Inject constructor(
    ...
    // ---- RESHAPE PAGE SIZE ------------------------------------------
    // Exports PDF with all pages scaled to a target page size using merge approach
        ...
        try {
            ...
            for (i in 1..src.numberOfPages) {
                ...
                try {
                    ...
                    // Use saveState/restoreState with cm operator for transformation
                    canvas.saveState()
                    canvas.concatMatrix(
                        scale.toDouble(), 0.0, 0.0, scale.toDouble(),
                        offX.toDouble(), offY.toDouble()
                    )
                    // addXObjectAt is available in iText7 Community
