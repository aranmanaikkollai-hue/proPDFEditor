...
class PdfOperationsManager @Inject constructor(
    ...
    suspend fun splitPdf(
        inputFile: File, outputDir: File, ranges: List<IntRange>
            ...
            try {
                ranges.forEachIndexed { i, range ->
                    val out  = File(outputDir,
                        "${inputFile.nameWithoutExtension}_part${i + 1}.pdf")
    ...
    suspend fun compressPdf(
        inputFile: File, outputFile: File, quality: Int = 1
    ...
    suspend fun removePdfPassword(
        ...
        runCatching {
            ...
            val doc = PdfDocument(
                PdfReader(inputFile.absolutePath, rp),
            ...
            try {
                ...
                for (i in 1..doc.numberOfPages) {
                    ...
                    val canvas = PdfCanvas(
                        page.newContentStreamBefore(), page.resources, doc
                    )
                    try {
                        canvas.saveState()
                            .setExtGState(gs)
    ...
    suspend fun deletePages(
        inputFile: File, outputFile: File, pagesToDelete: List<Int>
    ): Result<File> = withContext(Dispatchers.IO) {
        runCatching {
            val doc = PdfDocument(
                PdfReader(inputFile.absolutePath),
    ...
    suspend fun rotatePages(
        inputFile: File, outputFile: File, pages: Map<Int, Int>
    ): Result<File> = withContext(Dispatchers.IO) {
        runCatching {
            val doc = PdfDocument(
                PdfReader(inputFile.absolutePath),
        ...
        runCatching {
            ...
            try {
                ...
                for (i in 1..total) {
                    ...
                    val canvas = PdfCanvas(
                        page.newContentStreamAfter(), page.resources, doc
                    )
                    try {
                        canvas.beginText()
            ...
            try {
                ...
                for (i in 1..doc.numberOfPages) {
                    ...
                    try {
                        ...
                        headerText?.let {
                            canvas.moveText(cx, (ps.top - 20.0)).showText(it)
                        }
                        footerText?.let {
                            canvas.moveText(cx, (ps.bottom + 10.0)).showText(it)
                        }
                        canvas.endText()
                    } finally { canvas.release() }
                }
    ...
    suspend fun imagesToPdf(
        imageFiles: List<File>, outputFile: File
    ): Result<File> = withContext(Dispatchers.IO) {
        runCatching {
            ...
            try {
                imageFiles.forEach { f ->
                    pdfDoc.addNewPage()
                    document.add(
                        Image(ImageDataFactory.create(f.absolutePath))
                            .setAutoScale(true)
                    )
                }
            } finally {
                document.close() // also closes pdfDoc
            }
            outputFile
        }
    }
    ...
    suspend fun saveAnnotationsToPdf(
        inputFile: File,
        outputFile: File,
        pageAnnotations: Map<Int, Pair<List<com.propdf.editor.ui.viewer.AnnotationCanvasView.Stroke>, Float>>
    ): Result<File> = withContext(Dispatchers.IO) {
        runCatching {
            val doc = PdfDocument(
                PdfReader(inputFile.absolutePath),
                            ...
                            do {
                                ...
                                while (dist <= len) {
                                    if (measure.getPosTan(dist, pos, tan)) {
                                        ...
                                        if (first) {
                                            canvas.moveTo(px.toDouble(), py.toDouble())
                                            first = false
                                        } else {
