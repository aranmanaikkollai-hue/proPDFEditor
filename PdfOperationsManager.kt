...
/**
 * PdfOperationsManager — iText 7 Community, API 21+
 *
 * Rules:
 *  - PdfDocument is NOT Closeable → NO .use{} → manual try/finally + .close()
 *  - PdfCanvas.release() used to free canvas resources
 *  - PdfCanvasConstants REMOVED → raw PDF spec ints (1 = Round cap/join)
 *  - All ops on Dispatchers.IO
 *  - All ops return Result — no uncaught exceptions
 */
@Singleton
class PdfOperationsManager @Inject constructor(
    ...
    suspend fun mergePdfs(
        ...
        runCatching {
            val out = PdfDocument(PdfWriter(outputFile.absolutePath, compress()))
            try {
        ...
        runCatching {
            ...
            try {
                ranges.forEachIndexed { i, range ->
                    ...
                    val dest = PdfDocument(PdfWriter(out.absolutePath, compress()))
                    try {
    ...
    suspend fun compressPdf(
        ...
        runCatching {
            val doc = PdfDocument(PdfReader(inputFile.absolutePath), PdfWriter(outputFile.absolutePath, compress()))
    ...
    suspend fun removePdfPassword(
        ...
        runCatching {
            val rp  = ReaderProperties().setPassword(password.toByteArray())
            val doc = PdfDocument(PdfReader(inputFile.absolutePath, rp), PdfWriter(outputFile.absolutePath))
            try { } finally { doc.close() }
            outputFile
        }
    }
    ...
    suspend fun imagesToPdf(
        ...
        runCatching {
            val doc = PdfDocument(PdfWriter(outputFile.absolutePath, compress()))
            // Document wraps PdfDocument; Document.close() closes both
    ...
    suspend fun saveAnnotationsToPdf(
        ...
        runCatching {
            val doc = PdfDocument(PdfReader(inputFile.absolutePath), PdfWriter(outputFile.absolutePath, compress()))
            try {
                for ((pageIdx, data) in pageAnnotations) {
                    val (strokes, scale) = data
                    if (strokes.isEmpty()) continue
                    val pdfPage = doc.getPage(pageIdx + 1)
                    ...
                    try {
                        for (stroke in strokes) {
                            ...
                            pathToPdf(canvas, stroke.path, scale, pdfH)
                            canvas.stroke()
                            canvas.restoreState()
                        }
                    } finally {
    ...
    private fun applyStroke(canvas: PdfCanvas, stroke: AnnotationCanvasView.Stroke) {
        ...
        canvas.setLineCapStyle(1)   // 1 = Round (PDF spec, replaces PdfCanvasConstants)
        canvas.setLineJoinStyle(1)  // 1 = Round (PDF spec, replaces PdfCanvasConstants)
    }

    private fun pathToPdf(canvas: PdfCanvas, path: android.graphics.Path, scale: Float, pdfH: Float) {
        ...
        do {
            ...
            while (dist <= len) {
                if (measure.getPosTan(dist, pos, tan)) {
                    val px = pos[0] / scale; val py = pdfH - pos[1] / scale
                    if (first) { canvas.moveTo(px.toDouble(), py.toDouble()); first = false }
                    else canvas.lineTo(px.toDouble(), py.toDouble())
                }
                dist += 2f
            }
            if (measure.getPosTan(len, pos, tan))
                canvas.lineTo((pos[0] / scale).toDouble(), (pdfH - pos[1] / scale).toDouble())
        } while (measure.nextContour())
    }

    private fun compress() = WriterProperties().apply {
