...
// BUG FIXED: After zooming, panning offset was calculated relative to
the

// unscaled view origin, causing the content to jump to wrong position.

// FIX: Store the pivot point in scaled coordinates and translate around
it.
...
// Intercept multi-finger events for zoom; let single-finger pass to
children
...
// Convert screen touch coords to content coords (for annotation
placement)
...
Full ML Kit Text Recognition replacing the stub.
suspendCancellableCoroutine wrapper. PDF page-by-page OCR via
PdfRenderer at 2x resolution.
