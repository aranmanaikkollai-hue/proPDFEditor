 ...
 * - RGB_565 config = half the RAM of ARGB_8888
 * - Render jobs cancelled immediately on recycle (no wasted work)
 * - PdfRenderer synchronised — not thread safe, must not open 2 pages at once
 * - Night mode = ColorFilter on ImageView, zero extra allocation
 */
class PdfPageAdapter(
    ...
    var nightMode: Boolean = false
        set(v) { if (field != v) { field = v; notifyItemRangeChanged(0, itemCount, "night") } }
    ...
    inner class PageVH(val view: AnnotatedPageView) : RecyclerView.ViewHolder(view) {
        ...
        fun bind(pos: Int) {
            job?.cancel(); job = null
            view.setTool(activeTool, activeColor, activeWidth)
            applyNight()
            job = scope.launch {
                val bmp = render(pos)
    ...
    override fun onBindViewHolder(holder: PageVH, position: Int, payloads: MutableList<Any>) {
        if (payloads.isEmpty()) { holder.bind(position); return }
        payloads.forEach { p ->
            when (p) {
                "night" -> holder.applyNight()
                "tool"  -> holder.view.setTool(activeTool, activeColor, activeWidth)
            }
        }
    }
    ...
    fun setActiveTool(tool: String, color: Int, width: Float = 6f) {
        activeTool = tool; activeColor = color; activeWidth = width
        notifyItemRangeChanged(0, itemCount, "tool")
    }

    fun undoOnPage(pageIdx: Int, rv: RecyclerView) =
        (rv.findViewHolderForAdapterPosition(pageIdx) as? PageVH)?.view?.undoAnnotation()

    fun getAnnotationsForPage(
        pageIdx: Int, rv: RecyclerView
